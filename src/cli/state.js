...
async function ensureDirectory(directory) {
  ...
  if (process.platform === 'win32') {
    ...
    let expected = path.resolve(directory);
    // macOS exposes /var and /tmp through stable system-owned aliases to
    // /private/var and /private/tmp. Treat only those operating-system aliases
    // as canonical; a user-controlled symlink anywhere below them still
    // resolves to a different path and remains rejected.
    if (process.platform === 'darwin'
        && (/^\/var(?:\/|$)/.test(expected) || /^\/tmp(?:\/|$)/.test(expected))) {
...
async function reconcileOriginLocked(statePaths, config, origin) {
  ...
  if (marker === 'native' && !native.supported) {
    // A persisted native marker means a credential exists in the host's
    // operating-system store. Sandboxed or minimally provisioned processes
    // may lack that backend entirely; classify this as inaccessible so MCP
    // callers receive the exact host-execution fallback instead of a false
    // claim that the credential record is corrupt.
    throw new Error('Native credential store is inaccessible');
  }
