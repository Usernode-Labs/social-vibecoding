...
function registerTools(server, ctx) {
    ...
    return toolResult({
      ...
      nextStep: staleCheckoutWarning(checkout)
        ...
        + (result.proposalId
          ...
          : `submit_work with taskId ${result.taskId} and the patch (or branch) you produced. `)
  ...
  server.registerTool('submit_work', {
    ...
    description: "Turn finished work into a Homeroom proposal: opens the pull request, builds a staging preview, runs the app's checks and puts it to the group's vote. FOUR SHAPES, each complete as written — (1) `taskId` plus `patch`, the DEFAULT for a new change: Homeroom applies it at the recorded base commit in the app's own repository and opens the pull request itself, so NO fork or GitHub write access is needed; (2) `taskId` plus the `branch` you pushed, whatever it is called, for a patch over ~250 KB or branch-backed work; (3) `slug` plus `prNumber` for a pull request that is already open; (4) `proposalId` plus `branch` to UPDATE a proposal of the user's that is already up for a vote — for fixing a failing check or acting on review comments — which advances that same proposal onto your new commit instead of opening a second one, and clears the votes it has collected. Shape (4) needs no `slug`: naming the proposal names the app. When shape (4)'s target is a dev SESSION (a work-order continuation that is not yet up for a vote), it also takes `propose: true`: once the update lands, Homeroom promotes the session — the same act as the owner's Propose-to-group button, reopening a paused session first — so pass it only when the user has asked for the change to go to the vote; landing quietly stays the default. TWO DESTINATIONS: by default work goes up for a VOTE; `share: true` on shape (2) lands it in the app's IN-PROGRESS area instead \u2014 a shared session with a preview, no PR, no vote; the charter has the rule. A task belongs to the USER'S USERNODE ACCOUNT, not to one chat — any session connected as that account, including a coding agent's own connector, can submit it, and doing so is the expected path. Only work from the user's own GitHub account is submitted under their name.",
    inputSchema: {
      ...
      patch: z.string().optional()
        .describe('The change as a patch — the default way to submit a new change: the output of `git format-patch <baseSha>..HEAD --stdout`, or a plain `git diff`. Homeroom applies it at the task’s recorded base commit, commits it in the app’s own repository and opens the pull request, so you need no fork and no GitHub write access at all. Requires taskId. Roughly 250 KB max; push a branch for anything larger, and to revise an existing proposal (proposalId takes a branch, never a patch).'),
    ...
    if (!taskId && !prNumber && !updating && !(slug && branch)) {
      return toolError(
        ...
        + 'the branch you pushed; slug + prNumber for a pull request that is already open; or slug + branch, which recovers '
