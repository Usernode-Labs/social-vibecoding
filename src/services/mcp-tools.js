...
function registerTools(server, ctx) {
  ...
  // This description is the longest in the surface and it had drifted 112
  // chars OVER the 1800-char budget tests/mcp-instruction-budget.test.js
  // enforces — which is invisible in the product, because Claude Code cuts
  // every description at 2048 with no ellipsis and no log, and /mcp shows the
  // full text anyway.
  //
  // What came out is prose the CHARTER already carries verbatim, which is
  // where the test says a cross-cutting rule belongs: the enumerated
  // don't-touch-the-work-order list and the linked-GitHub precondition are
  // both in `work-order-handling` (mcp-charter.js), and the implicit claim is
  // in `saying-you-are-on-it`. The consequence of retyping the commit id
  // stays here, because that is the sentence a model needs at the moment it
  // is about to paste — the pointer beside it says where the full rule is.
  server.registerTool('prepare_work', {
    title: 'Prepare a change for a coding agent',
    description: "Prepare a change to a Usernode app for a coding agent. If you have repository, filesystem, shell or code-editing tools, YOU are that agent: execute `workOrder` in this conversation, implement and test the change, then call `submit_work` with your branch or patch — do not relay `guidance` or send the user elsewhere. Only if you lack those tools, show `guidance` — the human's next steps, already written for them — in order, as written, and reproduce `workOrder` in a fenced code block character for character, EXACTLY as returned. Do not shorten it, and do not retype the branch name or the 40-character commit id — one wrong character sends the coding agent to a starting point that does not exist. `get_connector_guidance` carries that rule in full. The work order names the app's repository, the fork to push to, the branch to create and the exact commit to start from; it makes the fork and the branch itself, because Usernode asks for NO write access to the user's GitHub account. Pass `proposalId` to REVISE a proposal that is already up for a vote instead of opening a new one — the work order is then based at that proposal's own head and its submission updates it in place. `openProposals` in the result names any proposals the group is ALREADY voting on for the same request: tell the user before they paste anything, because that work may be built already, and if one of them is theirs, calling prepare_work again with its `proposalId` continues it instead of opening a duplicate. Requires a linked GitHub account (identity only, so work can be attributed to them). This spends the user's own coding-agent subscription, not their Usernode credits.",
    inputSchema: {
      slug: z.string().describe('The app slug, as returned by list_apps.'),
