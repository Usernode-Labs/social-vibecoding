...
//
// Returns `{ title, body }`. Never throws — falls back to the legacy
// template so PR creation is never blocked on LLM availability.
...
async function applyPrMetadata({
  ...
  const { title: prTitle, body: prBody } = await generatePrMetadata({
