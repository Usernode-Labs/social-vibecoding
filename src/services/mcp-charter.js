...
const CHARTER_SECTIONS = Object.freeze([
  ...
  },
  {
    ...
    text: 'demo_mode, demo_propose, demo_vote, demo_reset and get_demo_status drive a RECORDING of the proposal flow. An app\'s creator switches it into demo mode and names a partner; that partner is a synthetic account the platform owns — it cannot sign in, and it acts only on apps in demo mode, only through these tools, and only at the creator\'s request. demo_propose opens a proposal from a branch already pushed to the app\'s repository, promotes it and sends the real vote notification; demo_vote casts the partner\'s vote through the real vote path; demo_reset removes the partner\'s proposals, votes and notifications on that app, puts main back to where it stood when demo mode was switched on, and redeploys. Two rules. First, never present the partner as a person: its proposals and votes are synthetic, the app\'s settings say so, and a viewer who asks is told so. Second, the platform refuses these tools on every app not in demo mode and on any app this user did not create — do not look for a way around that; there is none and there is not meant to be one. Call get_demo_status before a take: it lists exactly what would keep the notification or the vote from landing.',
  },
  {
