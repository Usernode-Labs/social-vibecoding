...
// Transcript rows written before the em-dash copy sweep carry the old
// " — rebuilding the preview now." tail, so the matcher accepts BOTH the
// old and the current wording indefinitely; dropping the old branch would
// make the backfill post a duplicate breadcrumb on every pre-sweep session.
