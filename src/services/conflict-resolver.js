...
async function resolveWithSessionInner(config, pool, session, options = {}, ctx = { step: async () => {} }) {
  ...
  if (pr0.merged) {
    ...
    await ctx.step({
      phase: 'pr_state',
      message: `PR #${session.pr_number} is already merged on GitHub, so the proposal is being marked merged.`,
  ...
  if (needsSync) {
    ...
    if (sync.syncResult === 'conflict') {
      ...
      await postGroupMessage(pool, session,
        `PR #${session.pr_number} couldn't be auto-merged with main, because Claude couldn't resolve the conflicts. ${owner}: open the session's dev-chat to resolve it.`
      );
      ...
      await ctx.step({ phase: 'group_chat', message: "Posted to group chat: couldn't auto-merge, so the owner must resolve it in dev-chat.", detail: { reason: 'unresolved_conflict' } });
  ...
  if (didSync && typeof mergeResult?.yesCount === 'number' && typeof mergeResult?.needed === 'number') {
    await postGroupMessage(pool, fresh,
      `PR #${fresh.pr_number} is now synced with main and conflict-free. It needs ${mergeResult.yesCount}/${mergeResult.needed} yes votes needed to merge.`
    );
  }
