...
//   USERNODE_PLATFORM_API_URL    — legacy, unversioned in-network base URL
//                                  of the app-facing read-only platform API.
//   USERNODE_PLATFORM_API_V1_URL — stable v1 base URL. New app code uses
//                                  this and may fall back to the legacy name.
// Token-gated routes on both are authenticated with the same token.
//
// STAGING DEPLOYS inject both API URL locators from staging-env.js but no
// LLM URL or app token — the exact `private: true` secret precedent:
// unreviewed PR code must not be able to spend grants or call token-gated
// platform APIs. Apps detect the absent credential and degrade those
// features (see app-conventions.md).
...
// #744: base URL of the app-facing read-only platform API. #1908 adds an
// explicit v1 variable while preserving this original unversioned value as a
// compatibility alias. Both use the SAME per-app token
...
async function productionLlmEnv(pool, appId) {
  try {
    ...
    return {
      ...
      // #744/#1908: stable and legacy app-facing read-only API bases,
      // authenticated with the same token. Staging injects the URLs but
      // deliberately omits this token.
