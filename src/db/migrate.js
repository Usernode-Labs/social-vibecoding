...
async function migrate(config) {
  ...
  await seedStagingCcBaselineEtaRun(pool, config);
...
// #906: baseline-countdown fixture. The always-present ETA is the readout a
// user WITHOUT the experimental estimator toggle sees, which means the one
// thing a reviewer cannot produce on demand is the interesting case —
// seedStagingCcEstimateRun above deliberately carries metadata.estimate, so
// it always exercises the AI path. These three runs carry NO estimate
// metadata at all, so the baseline is what renders, and they sit at three
// different rungs of the ladder:
//
//   mid    ~4 min elapsed  → ladder rung 397s → "~2m 30s left"
//   long   ~12 min elapsed → ladder rung 797s → "~1m 30s left", plus the
//                            10-minute cohort note beside it
//   commit ~5 min elapsed, log ends on [commit] → "under a minute left"
//                            (the phase marker overrides the ladder)
//
// chat_sessions is staging:private, so this is invisible without seeding.
// Idempotent on the fixture branch names; strict no-op in production.
async function seedStagingCcBaselineEtaRun(pool, config) {
  if (process.env.USERNODE_ENV !== 'staging') return;
  ...
  const appId = appRows[0]?.id;
  if (!appId) {
    log.warn('db', 'Staging CC-baseline-ETA fixture skipped: self-app row missing', {
      slug: config.selfAppSlug,
    });
    return;
  }
  ...
  if (!userRows.length) {
    log.warn('db', 'Staging CC-baseline-ETA fixture skipped: no users');
    return;
  }
  const owner = userRows[0];
  ...
  const baseLog = [
    '[refresh]',
    '[claude (mode build)]',
    '… Working out which files own the countdown',
  ...
  const runs = [
    {
      branch: 'staging-fixture/cc-baseline-eta-mid',
      title: '[staging fixture] Baseline countdown — mid-run',
      minutesAgo: 4,
      progressLog: baseLog,
    },
    {
      branch: 'staging-fixture/cc-baseline-eta-long',
      title: '[staging fixture] Baseline countdown — long run',
      minutesAgo: 12,
    ...
    {
      branch: 'staging-fixture/cc-baseline-eta-commit',
      title: '[staging fixture] Baseline countdown — wrapping up',
      minutesAgo: 5,
      progressLog: baseLog.concat(['[commit]']),
    },
  ];
  ...
  for (const run of runs) {
    ...
    const { rows: sessionRows } = await pool.query(
      `INSERT INTO chat_sessions
         (app_id, user_id, branch_name, pr_title, status, created_at)
       VALUES
         ($1, $2, $3, $4, 'active', NOW() - ($5::int * INTERVAL '1 minute'))
       RETURNING id`,
      [appId, owner.id, run.branch, run.title, run.minutesAgo + 1]
    );
    const sessionId = sessionRows[0].id;

    // Same ordering contract as seedStagingCcEstimateRun: the dev-chat
    // pairing pre-pass attaches the 'Claude Code progress' row to the
    // nearest PRECEDING active running status, and no terminal row is
    // written so the running line stays `_active`. The running line's
    // created_at is what the ETA ticks from, so its minutesAgo IS the
    // ladder rung this fixture demonstrates. Deliberately no
    // metadata.estimate anywhere — that is the whole point of the fixture.
    const messages = [
      {
        role: 'user',
        content: '[staging fixture] Please tighten up the progress readout.',
        metadata: {},
    ...
    log.info('db', 'Staging CC-baseline-ETA fixture seeded', {
      appId,
      owner: owner.username,
      sessionId,
      branch: run.branch,
