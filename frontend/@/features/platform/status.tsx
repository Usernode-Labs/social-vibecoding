...
export function StatusPage() {
  ...
  return <main className="isolate mx-auto flex w-full max-w-7xl flex-1 flex-col gap-6 px-4 py-8 antialiased sm:px-6" data-testid="operational-status"><header className="flex flex-wrap items-start justify-between gap-4"><div className="space-y-2"><h2 className="text-balance text-3xl font-semibold tracking-tight">Platform status</h2><p className="max-w-[62ch] text-base text-muted-foreground text-pretty">A public, read-only operational snapshot. It refreshes every five seconds and never exposes privileged controls.</p></div><Button onClick={() => setRefreshKey((value) => value + 1)} size="sm" type="button" variant="outline"><PlatformIcon data-icon="inline-start" icon={RefreshCw} />Refresh</Button></header>{state.kind === "loading" ? <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4"><Skeleton className="h-28" /><Skeleton className="h-28" /><Skeleton className="h-28" /><Skeleton className="h-28" /><Skeleton className="h-72 sm:col-span-2 xl:col-span-4" /></div> : null}{state.kind === "error" ? <Alert variant="destructive"><PlatformIcon icon={CircleAlert} /><AlertTitle>Status unavailable</AlertTitle><AlertDescription>{state.message}</AlertDescription></Alert> : null}{state.kind === "ready" ? <StatusContent snapshot={state.snapshot} /> : null}</main>
}
