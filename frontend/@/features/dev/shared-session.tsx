  ...
  return <div className="flex w-full flex-col gap-6">
    ...
    <Card>
      ...
      <CardContent className="flex flex-col gap-4">
        <div className="flex flex-wrap gap-2"><Badge variant="secondary"><PlatformIcon data-icon="inline-start" icon={RadioTower} />{session.status === "paused" ? "Paused" : "Live"}</Badge><Badge variant="outline">Visible to everyone</Badge><Badge variant="outline"><PlatformIcon data-icon="inline-start" icon={MessageCircle} />{Number(session.chat_count || 0)} discussion messages</Badge>{session.busy ? <Badge variant="outline">Working</Badge> : null}</div>
