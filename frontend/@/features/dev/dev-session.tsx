import { Archive, ArrowLeft, CheckCircle2, Eye, EyeOff, ExternalLink, FileText, FlaskConical, MessagesSquare, OctagonX, Pause, Play, RefreshCw, RotateCcw, Send } from "lucide-react"
...
export function DevSession() {
    ...
    try {
      ...
      if (result.keptPromoted) {
        ...
        setLifecycleNotice(result.ccPurged
          ? "Session restored. Its retained Builder workspace had expired, so the next turn starts with a fresh workspace."
          : result.prReopened
  ...
  const recheck = async () => {
    ...
    try {
      ...
      setRecheckNotice("Checks are running again. This session will refresh when the server publishes the result.")
      await reload()
    } catch (cause) {
  ...
  return <main className="isolate mx-auto flex w-full max-w-4xl flex-1 flex-col gap-6 px-4 py-8 antialiased sm:px-6" data-testid="dev-session">
    <Button className="w-fit" render={<Link to={`/apps/${encodeURIComponent(slug)}/dev`} />} variant="ghost"><PlatformIcon data-icon="inline-start" icon={ArrowLeft} />All sessions</Button>
    ...
    {data ? <>
      {useFixture ? <Alert><AlertTitle>Development fixture</AlertTitle><AlertDescription>This transcript is deterministic review data; production requests the existing session API.</AlertDescription></Alert> : null}
      {isProductionReadOnlyReview ? <Alert><AlertTitle>Production review mode</AlertTitle><AlertDescription>This local React review can read production data, but sending messages and uploading attachments are disabled.</AlertDescription></Alert> : null}
      <header className="flex flex-wrap items-start justify-between gap-4"><div className="min-w-0 space-y-2"><div className="flex flex-wrap items-center gap-2"><h2 className="text-balance text-3xl font-semibold tracking-tight">{sessionTitle(data.session)}</h2><Badge variant={data.session.status === "active" ? "secondary" : "outline"}>{data.session.status}</Badge>{isShared ? <Badge variant="outline">Visible to everyone</Badge> : null}</div><p className="text-base text-muted-foreground">{data.session.branch_name || "No branch"}</p></div><div className="flex flex-wrap gap-2"><Button render={<Link to={`/apps/${encodeURIComponent(slug)}/dev/sessions/${encodeURIComponent(sessionId)}/spec`} />} variant="outline"><PlatformIcon data-icon="inline-start" icon={FileText} />Spec</Button><Button render={<Link to={`/apps/${encodeURIComponent(slug)}/dev/sessions/${encodeURIComponent(sessionId)}/preview`} />} variant="outline"><PlatformIcon data-icon="inline-start" icon={FlaskConical} />Preview</Button>{!canUnarchive ? <Button disabled={isProductionReadOnlyReview || visibilityUpdating} onClick={() => void updateVisibility()} type="button" variant="outline"><PlatformIcon data-icon="inline-start" icon={isShared ? EyeOff : Eye} />{visibilityUpdating ? "Updating…" : isShared ? "Make private" : "Make visible"}</Button> : null}{canPromote ? <AlertDialog open={promoteOpen} onOpenChange={setPromoteOpen}><AlertDialogTrigger disabled={isProductionReadOnlyReview || promoting} render={<Button />}><PlatformIcon data-icon="inline-start" icon={Send} />Propose change</AlertDialogTrigger><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Propose these changes for voting?</AlertDialogTitle><AlertDialogDescription>This publishes the current session as a proposal. The server checks your proposal capacity, prepares its pull request, and starts the normal review lifecycle.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel disabled={promoting}>Cancel</AlertDialogCancel><AlertDialogAction disabled={promoting} onClick={() => void promote()}>{promoting ? "Proposing…" : "Propose change"}</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog> : null}{canStop ? <Button disabled={isProductionReadOnlyReview || stopping} onClick={() => void stopTurn()} type="button" variant="outline"><PlatformIcon data-icon="inline-start" icon={OctagonX} />{stopping ? "Stopping…" : "Stop turn"}</Button> : null}{canPause ? <Button disabled={isProductionReadOnlyReview || Boolean(lifecycleAction)} onClick={() => void updateLifecycle("pause")} type="button" variant="outline"><PlatformIcon data-icon="inline-start" icon={Pause} />{lifecycleAction === "pause" ? (data.session.status === "promoted" ? "Freeing…" : "Pausing…") : pauseLabel}</Button> : null}{canResume ? <Button disabled={isProductionReadOnlyReview || Boolean(lifecycleAction)} onClick={() => void updateLifecycle("resume")} type="button"><PlatformIcon data-icon="inline-start" icon={Play} />{lifecycleAction === "resume" ? "Resuming…" : "Resume session"}</Button> : null}{canArchive ? <AlertDialog open={archiveOpen} onOpenChange={setArchiveOpen}><AlertDialogTrigger disabled={isProductionReadOnlyReview || Boolean(lifecycleAction)} render={<Button variant="destructive" />}><PlatformIcon data-icon="inline-start" icon={Archive} />Archive session</AlertDialogTrigger><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Archive this session?</AlertDialogTitle><AlertDialogDescription>This stops its worker, closes its pull request, and frees the session slot. You can restore it here for a limited time.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel disabled={lifecycleAction === "archive"}>Cancel</AlertDialogCancel><AlertDialogAction disabled={lifecycleAction === "archive"} onClick={() => void updateLifecycle("archive")} variant="destructive">{lifecycleAction === "archive" ? "Archiving…" : "Archive session"}</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog> : null}<Button render={<a aria-label="Continue this session in the legacy Dev workspace" href={legacyHash(`#app/${encodeURIComponent(slug)}/dev/sessions/${encodeURIComponent(sessionId)}`)} />} variant="outline"><PlatformIcon data-icon="inline-start" icon={ExternalLink} />Continue in legacy Dev</Button></div></header>
      {canRecheck ? <Alert>
        ...
        <AlertDescription className="flex flex-wrap items-center justify-between gap-3">{data.session.check_state === "pending" ? "The last check run has not reported a final result yet. You can ask the server to coalesce another check run if it appears stuck." : data.session.check_error_detail || "Ask the server to rebuild staging if needed and run the existing proposal checks again."}<AlertDialog open={recheckOpen} onOpenChange={setRecheckOpen}><AlertDialogTrigger disabled={isProductionReadOnlyReview || rechecking} render={<Button size="sm" variant="outline" />}><PlatformIcon data-icon="inline-start" icon={rechecking ? RefreshCw : CheckCircle2} className={rechecking ? "animate-spin" : undefined} />{rechecking ? "Starting checks…" : "Re-run checks"}</AlertDialogTrigger><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Run the proposal checks again?</AlertDialogTitle><AlertDialogDescription>The server may rebuild the staging preview and run its existing browser checks. It coalesces duplicate requests and updates the canonical check state when the run finishes.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel disabled={rechecking}>Cancel</AlertDialogCancel><AlertDialogAction disabled={rechecking} onClick={() => void recheck()}>{rechecking ? "Starting…" : "Re-run checks"}</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog></AlertDescription>
      </Alert> : null}
      ...
      {canUnarchive ? <Alert>
        ...
        <AlertDescription className="flex flex-wrap items-center justify-between gap-3">Restore it to paused when you are ready to continue. The server decides whether its pull request can reopen and whether the retained Builder workspace is still available.<AlertDialog open={unarchiveOpen} onOpenChange={setUnarchiveOpen}><AlertDialogTrigger disabled={isProductionReadOnlyReview || Boolean(lifecycleAction)} render={<Button size="sm" />}><PlatformIcon data-icon="inline-start" icon={RotateCcw} />Restore session</AlertDialogTrigger><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>Restore this archived session?</AlertDialogTitle><AlertDialogDescription>The server restores it to paused and attempts to reopen its pull request. If the retained Builder workspace has expired, the next turn starts with a fresh workspace.</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel disabled={lifecycleAction === "unarchive"}>Cancel</AlertDialogCancel><AlertDialogAction disabled={lifecycleAction === "unarchive"} onClick={() => void updateLifecycle("unarchive")}>{lifecycleAction === "unarchive" ? "Restoring…" : "Restore session"}</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog></AlertDescription>
      </Alert> : null}
      ...
      {liveMessages.length ? <><DevConversation messages={liveMessages} sessionId={sessionId} streamState={streamState} /><DevComposer disabled={composerDisabled} onTurnStarted={reload} quickReplies={quickReplies} sessionId={sessionId} streaming={composerStreaming} suggestions={suggestions} /></> : <><Empty><EmptyHeader><EmptyMedia variant="icon"><PlatformIcon icon={MessagesSquare} /></EmptyMedia><EmptyTitle>{canUnarchive ? "Restore this session to continue" : "No messages yet"}</EmptyTitle><EmptyDescription>{canUnarchive ? "Archived sessions preserve their history but cannot receive a new Builder turn until restored." : "Start this session with Builder below."}</EmptyDescription></EmptyHeader></Empty><DevComposer disabled={composerDisabled} onTurnStarted={reload} quickReplies={quickReplies} sessionId={sessionId} streaming={composerStreaming} suggestions={suggestions} /></>}
    </> : null}
  </main>
}
