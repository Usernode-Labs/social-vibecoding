...
export function DevBuildTimeline({ estimate, liveLines = [], messages }: { estimate?: string | null; liveLines?: string[]; messages: DevSessionMessage[] }) {
  ...
  return <Card data-testid="dev-build-timeline"><CardHeader><CardTitle className="flex items-center gap-2"><PlatformIcon icon={Hammer} />Build timeline</CardTitle>{estimate ? <Badge variant="secondary"><PlatformIcon data-icon="inline-start" icon={Timer} />{estimate}</Badge> : null}</CardHeader><CardContent className="space-y-2">{segments.map((segment) => <details className="rounded-md border" key={segment.id} open={segment.active}><summary className="flex cursor-pointer items-center gap-2 p-3 text-sm font-medium"><PlatformIcon icon={ChevronRight} size="xs" />{segment.label}<span className="ml-auto text-xs text-muted-foreground">{segment.lines.length} lines</span></summary><pre className="max-h-72 overflow-auto border-t bg-container p-3 text-xs whitespace-pre-wrap" data-surface="container">{segment.lines.join("\n")}</pre></details>)}</CardContent></Card>
}
