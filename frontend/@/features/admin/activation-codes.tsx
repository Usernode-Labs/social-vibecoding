import { Copy, ExternalLink, KeyRound, Plus, RefreshCw, ShieldAlert, Trash2 } from "lucide-react"
...
export function ActivationCodesPage() {
  ...
  return <main className="isolate mx-auto flex w-full max-w-3xl flex-1 flex-col gap-6 px-4 py-8 antialiased sm:px-6" data-testid="activation-codes">
    <header className="flex flex-wrap items-start justify-between gap-4"><div className="space-y-2"><h2 className="text-balance text-3xl font-semibold tracking-tight">Activation codes</h2><p className="text-base text-muted-foreground text-pretty">Create, copy, and revoke registration codes here. Account-role and deletion controls remain in legacy management.</p></div>{state.kind === "ready" ? <div className="flex flex-wrap gap-2"><Button disabled={!canCreate || creating} onClick={() => void create()} type="button"><PlatformIcon data-icon="inline-start" icon={Plus} />{creating ? "Generating…" : "Generate code"}</Button><Button onClick={() => setReloadToken((value) => value + 1)} type="button" variant="outline"><PlatformIcon data-icon="inline-start" icon={RefreshCw} />Refresh</Button></div> : null}</header>
    ...
    {isProductionReadOnlyReview && state.kind === "ready" ? <Alert><PlatformIcon icon={ShieldAlert} /><AlertTitle>Production review mode</AlertTitle><AlertDescription>Activation codes can be reviewed here, but generating and revoking codes are disabled.</AlertDescription></Alert> : null}
    ...
    {state.kind === "ready" ? <><div className="flex justify-end"><Button render={<a href="/#admin/codes" />} variant="outline">Open legacy management<PlatformIcon data-icon="inline-end" icon={ExternalLink} /></Button></div><ActivationCodesList canRevoke={canRevoke} codes={codes} copiedCodeId={copiedCodeId} onCopy={(code) => void copy(code)} onRevoke={setRevocationTarget} />
