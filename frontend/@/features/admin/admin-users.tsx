...
export function AdminUsersPage() {
  ...
  return <div className="isolate flex w-full flex-1 flex-col" data-testid="admin-users">
    ...
    {state.kind === "ready" ? <><div className="flex flex-wrap items-center justify-between gap-3"><Input aria-label="Filter users" className="max-w-sm" onChange={(event) => setQuery(event.target.value)} placeholder="Filter users" type="search" value={query} /><ActionAnchor href="/#admin/users">Manage accounts<PlatformIcon data-icon="inline-end" icon={ExternalLink} /></ActionAnchor></div>{!users.length ? <Empty><EmptyHeader><EmptyMedia variant="icon"><PlatformIcon icon={UsersRound} /></EmptyMedia><EmptyTitle>No matching users</EmptyTitle><EmptyDescription>Try another name, or clear the filter.</EmptyDescription></EmptyHeader></Empty> : <section aria-label="Platform users" className="space-y-3">{users.map((user) => <UserCard canWrite={canWrite} key={user.id} onUpdate={updateUser} user={user} />)}</section>}</> : null}
  </div></div>
}
