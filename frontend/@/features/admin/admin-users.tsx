import { ExternalLink, RefreshCw, Save, ShieldAlert, UsersRound } from "lucide-react"
...
export function AdminUsersPage() {
  ...
  return <main className="isolate mx-auto flex w-full max-w-5xl flex-1 flex-col gap-6 px-4 py-8 antialiased sm:px-6" data-testid="admin-users">
    <header className="flex flex-wrap items-start justify-between gap-4"><div className="space-y-2"><h2 className="text-balance text-3xl font-semibold tracking-tight">Users</h2><p className="text-base text-muted-foreground text-pretty">Inspect accounts and update app quotas or daily spend overrides. Role, deletion, password, and wallet administration remain in legacy management.</p></div>{state.kind === "ready" ? <Button onClick={() => setReloadToken((value) => value + 1)} type="button" variant="outline"><PlatformIcon data-icon="inline-start" icon={RefreshCw} />Refresh</Button> : null}</header>
    ...
    {state.kind === "ready" && isProductionReadOnlyReview ? <Alert><PlatformIcon icon={ShieldAlert} /><AlertTitle>Production review mode</AlertTitle><AlertDescription>Users can be reviewed here, but quota and spend changes are disabled.</AlertDescription></Alert> : null}
    {state.kind === "ready" ? <><div className="flex flex-wrap items-center justify-between gap-3"><Input aria-label="Filter users" className="max-w-sm" onChange={(event) => setQuery(event.target.value)} placeholder="Filter users" value={query} /><Button render={<a href="/#admin/users" />} variant="outline">Open legacy management<PlatformIcon data-icon="inline-end" icon={ExternalLink} /></Button></div>{!users.length ? <Empty><EmptyHeader><EmptyMedia variant="icon"><PlatformIcon icon={UsersRound} /></EmptyMedia><EmptyTitle>No matching users</EmptyTitle><EmptyDescription>Try another name, or clear the filter.</EmptyDescription></EmptyHeader></Empty> : <section aria-label="Platform users" className="space-y-3">{users.map((user) => <UserCard canWrite={canWrite} key={user.id} onUpdate={updateUser} user={user} />)}</section>}</> : null}
  </main>
}
