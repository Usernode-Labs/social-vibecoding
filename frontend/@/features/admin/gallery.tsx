import { ExternalLink, Image, RefreshCw, ShieldAlert } from "lucide-react"
...
function GalleryProposalCard({ proposal }: { proposal: GalleryProposal }) {
  ...
  return <article><Card><CardHeader className="gap-3"><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><CardTitle className="text-balance text-xl tracking-tight">{proposal.title || `Proposal ${proposal.id}`}</CardTitle><CardDescription>{appLabel} · {date(proposal.mergedAt)}</CardDescription></div><Badge variant={state.variant}>{state.label}</Badge></div><div className="flex flex-wrap gap-2"><Button render={proposal.appSlug ? <Link to={appDevPath(proposal.appSlug)} /> : <span />} size="sm" variant="outline" disabled={!proposal.appSlug}>Open app Dev</Button><Button render={proposal.appSlug ? <Link to={appDevProposalPath(proposal.appSlug, proposal.id)} /> : <span />} size="sm" variant="outline" disabled={!proposal.appSlug}>Open proposal</Button>{proposal.prUrl ? <Button render={<a href={proposal.prUrl} rel="noopener noreferrer" target="_blank" />} size="sm" variant="outline">{proposal.prNumber == null ? "Open pull request" : `PR #${proposal.prNumber}`}<PlatformIcon data-icon="inline-end" icon={ExternalLink} /></Button> : null}</div>{proposal.captureReason ? <CardDescription>{proposal.captureReason}</CardDescription> : null}</CardHeader><CardContent><GalleryVisualEvidence visuals={proposal.visuals} /></CardContent></Card></article>
}
...
function GalleryContent() {
  ...
  return <main className="isolate mx-auto flex w-full max-w-6xl flex-1 flex-col gap-6 px-4 py-8 antialiased sm:px-6" data-testid="admin-gallery"><header className="flex flex-wrap items-start justify-between gap-4"><div className="space-y-2"><h2 className="text-balance text-3xl font-semibold tracking-tight">Screenshot gallery</h2><p className="max-w-[64ch] text-base/7 text-muted-foreground text-pretty sm:text-sm/6">Before-and-after review evidence for merged proposals. Recordings play only when you choose them.</p></div><div className="flex flex-wrap gap-2"><Button render={<a href="/gallery" />} size="sm" variant="outline">Open legacy gallery<PlatformIcon data-icon="inline-end" icon={ExternalLink} /></Button><Button onClick={() => setReload((value) => value + 1)} size="sm" type="button" variant="outline"><PlatformIcon data-icon="inline-start" icon={RefreshCw} />Refresh</Button></div></header><GalleryFiltersPanel apps={apps} disabled={pageState.loading} draft={draft} onChange={setDraft} onSubmit={() => setFilters(draft)} />{appsError ? <p className="text-base/7 text-muted-foreground sm:text-sm/6" role="status">App filters are unavailable. You can still review all proposals.</p> : null}{stats ? <GalleryStatsStrip stats={stats} /> : null}{statsError ? <p className="text-base/7 text-muted-foreground sm:text-sm/6" role="status">{statsError}</p> : null}{pageState.loading && !pageState.page ? <div className="flex flex-col gap-4"><Skeleton className="h-28" /><Skeleton className="h-96" /><Skeleton className="h-96" /></div> : null}{pageState.error ? <Alert variant="destructive"><PlatformIcon icon={ShieldAlert} /><AlertTitle>Screenshot gallery unavailable</AlertTitle><AlertDescription>{pageState.error}</AlertDescription></Alert> : null}{pageState.page ? <><GalleryProposalList proposals={pageState.page.proposals} />{pageState.page.hasMore ? <div className="flex flex-col items-center gap-3"><Button disabled={loadingOlder} onClick={() => void loadOlder()} type="button" variant="outline">{loadingOlder ? "Loading older proposals…" : "Load older"}</Button>{olderError ? <p className="text-base/7 text-muted-foreground sm:text-sm/6" role="status">Older proposals could not be loaded. The proposals above are unchanged.</p> : null}</div> : null}</> : null}</main>
}
...
export function GalleryPage() {
  ...
  return <main className="isolate mx-auto flex w-full max-w-6xl flex-1 flex-col gap-6 px-4 py-8 antialiased sm:px-6" data-testid="admin-gallery"><header className="space-y-2"><h2 className="text-balance text-3xl font-semibold tracking-tight">Screenshot gallery</h2><p className="text-base/7 text-muted-foreground text-pretty sm:text-sm/6">Before-and-after review evidence for merged proposals.</p></header>{access.kind === "loading" ? <div className="flex flex-col gap-4"><Skeleton className="h-24" /><Skeleton className="h-72" /></div> : null}{access.kind === "denied" ? <Alert><PlatformIcon icon={ShieldAlert} /><AlertTitle>Admin access required</AlertTitle><AlertDescription>{access.message}</AlertDescription></Alert> : null}{access.kind === "error" ? <Alert variant="destructive"><AlertTitle>Screenshot gallery unavailable</AlertTitle><AlertDescription>{access.message}</AlertDescription></Alert> : null}</main>
}
