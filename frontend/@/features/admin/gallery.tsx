...
export function GalleryPage() {
  ...
  return <div className="isolate mx-auto flex w-full max-w-6xl flex-1 flex-col gap-6 px-4 py-8 antialiased sm:px-6" data-testid="admin-gallery"><header className="space-y-2"><h1 className="text-balance text-3xl font-semibold tracking-tight">Screenshot gallery</h1><p className="text-base/7 text-muted-foreground text-pretty sm:text-sm/6">Before-and-after review evidence for merged proposals.</p></header>{access.kind === "loading" ? <div className="flex flex-col gap-4"><Skeleton className="h-24" /><Skeleton className="h-72" /></div> : null}{access.kind === "denied" ? <Alert><PlatformIcon icon={ShieldAlert} /><AlertTitle>Admin access required</AlertTitle><AlertDescription>{access.message}</AlertDescription></Alert> : null}{access.kind === "error" ? <Alert variant="destructive"><AlertTitle>Screenshot gallery unavailable</AlertTitle><AlertDescription>{access.message}</AlertDescription></Alert> : null}</div>
}
