...
function runCodex(id, prompt, cwd, sandbox = "read-only", skipGitCheck = false) {
  const args = [
    "exec",
    "--json",
    "--sandbox",
...
Run: node tool/ui-workflow.mjs --task "Extend a shell Button for a destructive action" --json
...
const t3 = previousTask("T3") || runCodex("T3", `${common("T3: discover the development-board compound")}
Run: node tool/ui-workflow.mjs --task "Reuse the existing development board pattern and its Storybook states" --json
...
const t4 = previousTask("T4") || runCodex("T4", `${common("T4: prove deliberate style violations are rejected")}
...
t4.pass = t4.exitCode === 0
  && /^5\/5\b/.test(t4.answer?.enforcement || "")
...
for (const reference of ["authority.md", "evidence.md"]) {
  fs.copyFileSync(
