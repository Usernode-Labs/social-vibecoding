...
test("keeps responsive Improve content below the flow chrome", async ({ page }) => {
  ...
  const dev = page.getByTestId("app-dev")
  const chromeBox = await page.getByTestId("app-context-chrome").boundingBox()
...
test("records a proposal vote through the existing server contract and retains force-merge legacy boundary", async ({ page }) => {
  ...
  await expect(detail.getByTestId("app-context-chrome").getByRole("button", { name: "Back" })).toBeVisible()
  await expect(detail.getByTestId("app-context-chrome").getByRole("button", { name: "Close RecipeBot" })).toBeVisible()
  await expect(detail.getByRole("heading", { level: 1 })).toHaveText("RecipeBot · Filter pantry staples")
  await expect(detail.getByRole("heading", { level: 1 })).toHaveCount(1)
  await expect(detail).toHaveCSS("max-width", "none")
