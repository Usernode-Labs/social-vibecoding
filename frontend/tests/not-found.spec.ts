...
test("turns an unmatched React deep link into a recoverable page", async ({ page }) => {
  ...
  const homeLink = route.getByRole("link", { name: "Go to Home" })
  await expect(homeLink).toHaveAttribute("data-slot", "action-link")
  await expect(homeLink).toHaveAttribute("href", "/react")
