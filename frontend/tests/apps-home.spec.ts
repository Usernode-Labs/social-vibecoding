...
test("uses the detail view for primary app destinations", async ({ page }) => {
  ...
  const openApp = page.getByRole("link", { name: "Open RecipeBot" })
  await expect(openApp).toHaveAttribute("data-slot", "action-link")
  await expect(openApp).toHaveAttribute("href", "/react/apps/recipebot/open")
  const improveApp = page.getByRole("link", { name: "Improve RecipeBot" })
  ...
  const collaborators = page.getByRole("link", { name: "Manage RecipeBot collaborators" })
  await expect(collaborators).toHaveAttribute("data-slot", "action-link")
  await expect(collaborators).toHaveAttribute("href", "/react/apps/recipebot/members")
...
test("saves an app from its detail action hub and reflects the server-confirmed state", async ({ page }) => {
  ...
  const unavailableApp = page.getByRole("link", { name: "Open Game Corner" })
  await expect(unavailableApp).toHaveAttribute("data-slot", "action-link")
  await expect(unavailableApp).toHaveAttribute("href", "/react/apps/game-corner/open")
  await expect(unavailableApp).toHaveAttribute("aria-disabled", "true")
  await expect(unavailableApp).toHaveAttribute("tabindex", "-1")
  await unavailableApp.dispatchEvent("click")
  await expect(page).toHaveURL(/\/react\/apps\/game-corner$/)
