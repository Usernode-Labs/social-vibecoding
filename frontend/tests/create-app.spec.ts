...
test("checks repository access before importing and uses the confirmed source", async ({ page }) => {
  ...
  await page.goto("/react/create")
  await page.getByRole("radio", { name: /Import existing/ }).click()
...
test("surfaces preflight errors and never creates an unchecked imported app", async ({ page }) => {
  ...
  await page.getByLabel("GitHub repository URL").fill("https://github.com/ava/recipes")
  await page.getByRole("button", { name: "Check access" }).click()
  const repositoryField = page.getByTestId("repository-access-field")
  await expect(repositoryField.getByRole("alert")).toHaveText("Invite usernode-bot with Write access first.")
  await expect(page.getByRole("alert").filter({ hasText: "Could not create app" })).toHaveCount(0)
...
test("does not expose a creation write for an account without an available slot", async ({ page }) => {
  ...
  await page.goto("/react/create")
  const blocked = page.getByTestId("create-app-blocked")
  await expect(blocked).toContainText("App creation unavailable")
  await expect(blocked.getByRole("link", { name: "Back to apps" })).toBeVisible()
  await expect(page.getByRole("form", { name: "Create an app" })).toHaveCount(0)
  expect(writes).toBe(0)
})

test("shows access loading before revealing the creation decisions", async ({ page }) => {
  let releaseAccess!: () => void
  await page.route("**/api/auth/me", async (route) => {
    await new Promise<void>((resolve) => { releaseAccess = resolve })
    await route.fulfill({ json: { user: { id: 7, username: "ava", canCreateApps: true } } })
  })
  await page.goto("/react/create")
  await expect(page.getByRole("status")).toHaveText(/Checking app creation access/)
  await expect(page.getByRole("form", { name: "Create an app" })).toHaveCount(0)
  releaseAccess()
  await expect(page.getByRole("form", { name: "Create an app" })).toBeVisible()
})
...
test("is usable at a narrow mobile viewport and has no critical accessibility violations", async ({ page }) => {
  ...
  await expect(page.getByRole("radio", { name: /Create new/ })).toBeVisible()
...
test("production review mode disables app creation without a mutation request", async ({ page }) => {
  ...
  await page.goto("/react/create")
  const blocked = page.getByTestId("create-app-blocked")
  await expect(blocked).toContainText("Production review mode")
  await expect(page.getByRole("form", { name: "Create an app" })).toHaveCount(0)
  expect(writes).toBe(0)
})
