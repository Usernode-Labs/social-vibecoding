...
test("renders view-authorized shared metadata and its public session discussion", async ({ page }) => {
  ...
  const linkedIssue = detail.getByRole("link", { name: "Issue #84" })
  await expect(linkedIssue).toHaveAttribute("data-slot", "badge")
  await expect(linkedIssue).toHaveAttribute("href", "/react/apps/recipebot/dev/issues/84")
  await expect(linkedIssue).not.toHaveAttribute("target")
  await expect(linkedIssue).not.toHaveAttribute("rel")
  await expect(linkedIssue).not.toHaveAttribute("download")
  ...
  const livePreview = detail.getByRole("link", { name: "Open live preview" })
  await expect(livePreview).toHaveAttribute("data-slot", "action-anchor")
  await expect(livePreview).toHaveAttribute("href", session.staging_url)
  await expect(livePreview).toHaveAttribute("target", "_blank")
  await expect(livePreview).toHaveAttribute("rel", "noreferrer")
