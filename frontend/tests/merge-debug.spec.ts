...
test("renders any-admin read-only diagnostics and expands an ordered trace", async ({ page }) => { await page.goto("/react/admin/debug"); const route = page.getByTestId("merge-debug"); await expect(route).not.toHaveClass(/(?:mx-auto|max-w-)/); await expect(page.getByRole("heading", { exact: true, level: 1, name: "Merge debug" })).toHaveCount(1); await expect(route).toContainText("RecipeBot · PR #48"); await page.getByRole("button", { name: "View trace" }).click(); await expect(route).toContainText("Checks passed."); const debugLink = page.getByRole("link", { name: "Open debug tools" }); await expect(debugLink).toHaveAttribute("data-slot", "action-anchor"); await expect(debugLink).toHaveAttribute("href", "/debug") })
