...
const Secrets = {
  ...
  renderRow(s, canWrite) {
    ...
    if (isGithubRow) {
      ...
      valueDisplay = `<span class="text-xs font-medium text-red-700 dark:text-red-400">missing, so deploys are blocked</span>`;
  ...
  validateDeclaration(f, isPlatform) {
    ...
    if (isPlatform && f.value && /['\r]/.test(f.value)) {
      return "Values can't contain a single quote or a carriage return, because they wouldn't survive being "
  ...
  async handleSet(key, sensitive) {
    const value = await PlatformUI.prompt({
      title: `Set ${key}`,
      message: sensitive ? 'This value is sensitive, so it is encrypted at rest and never shown again.' : undefined,
      placeholder: 'value',
      confirmLabel: 'Save',
    });
