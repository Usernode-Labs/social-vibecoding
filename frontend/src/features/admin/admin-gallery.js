...
const AdminGallery = (() => {
  ...
  function proposalCardHtml(p) {
    ...
    const appLink = p.appSlug
      ? `<a href="/#app/${esc(p.appSlug)}/dev" class="text-violet-500 dark:text-violet-400 hover:text-violet-800 dark:hover:text-violet-300 dark:hover:text-violet-300">${esc(appLabel)}</a>`
      : esc(appLabel);
    const prLink = (p.prUrl && p.prNumber)
      ...
      : (p.prNumber ? `<span class="font-mono text-zinc-500 dark:text-zinc-400">PR#${esc(p.prNumber)}</span>` : '');
    const proposalLink = p.appSlug
      ? `<a href="/#app/${esc(p.appSlug)}/dev/proposals/${p.id}" class="text-violet-500 dark:text-violet-400 hover:text-violet-800 dark:hover:text-violet-300 dark:hover:text-violet-300">Open proposal &rarr;</a>`
      : '';
  ...
  const MARKUP = `
    <div id="admin-gallery-root">
      ...
      <div id="admin-gallery-gate" class="hidden text-zinc-500 dark:text-zinc-400 text-center py-20"></div>
      ...
      <main id="admin-gallery-content" class="hidden space-y-4">
        ...
        <div class="flex justify-center py-4">
          ...
          <span id="admin-gallery-empty" class="hidden text-zinc-500 dark:text-zinc-400 text-sm">No merged proposals match these filters yet.</span>
        </div>
      </main>
    </div>`;
