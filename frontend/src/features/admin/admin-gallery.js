...
const AdminGallery = (() => {
  ...
  const CHIP = {
    ...
    console_only: { label: 'No visual change expected', cls: 'bg-gray-500/15 text-gray-500 dark:text-gray-400' },
    failed: { label: 'Capture failed', cls: 'bg-rose-500/15 text-rose-600 dark:text-rose-400' },
  };

  function chipHtml(state, reason) {
    const chip = CHIP[state] || { label: 'Outcome unknown', cls: 'bg-gray-500/15 text-gray-500 dark:text-gray-400' };
  ...
  function proposalCardHtml(p) {
    ...
    const appLink = p.appSlug
      ? `<a href="/#app/${esc(p.appSlug)}/dev" class="text-indigo-500 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 dark:hover:text-indigo-300">${esc(appLabel)}</a>`
      : esc(appLabel);
    const prLink = (p.prUrl && p.prNumber)
      ? `<a href="${esc(p.prUrl)}" target="_blank" rel="noopener" class="font-mono text-indigo-500 dark:text-indigo-400 hover:underline">PR#${esc(p.prNumber)}</a>`
      : (p.prNumber ? `<span class="font-mono text-gray-500">PR#${esc(p.prNumber)}</span>` : '');
    const proposalLink = p.appSlug
      ? `<a href="/#app/${esc(p.appSlug)}/dev/proposals/${p.id}" class="text-indigo-500 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 dark:hover:text-indigo-300">Open proposal &rarr;</a>`
      : '';
    const meta = [prLink, fmtDate(p.mergedAt) ? esc(fmtDate(p.mergedAt)) : '', proposalLink]
      .filter(Boolean).join(' <span class="text-gray-400 dark:text-gray-600">·</span> ');
    ...
    const body = tiles || `<div class="text-xs text-gray-500 dark:text-gray-400 py-2">
    ...
    return `<article class="${AdminUI.card} p-3">
      <div class="flex items-start justify-between gap-3 mb-1">
        <div class="min-w-0">
          ...
          <div class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">${appLink} <span class="text-gray-400 dark:text-gray-600">·</span> ${meta}</div>
        </div>
  ...
  function statsHtml(s) {
    ...
    const item = (label, n, withPct) => `<span><strong class="text-gray-700 dark:text-gray-200">${n}</strong> ${esc(label)}${withPct && s.total ? ` (${pct(n)}%)` : ''}</span>`;
    return [
  ...
  const MARKUP = `
    <div id="admin-gallery-root">
      ...
      <div id="admin-gallery-gate" class="hidden text-gray-500 text-center py-20"></div>
      ...
      <main id="admin-gallery-content" class="hidden space-y-4">
        ...
        <section class="${AdminUI.card} p-3
          ...
          <label class="flex flex-col text-xs text-gray-500 dark:text-gray-400">App
            <select id="admin-gallery-f-app" class="mt-1 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded px-2 py-1 text-sm text-gray-900 dark:text-gray-100">
              <option value="">All apps</option>
            </select>
          </label>
          <label class="flex flex-col text-xs text-gray-500 dark:text-gray-400">Problem
            <select id="admin-gallery-f-problem" class="mt-1 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded px-2 py-1 text-sm text-gray-900 dark:text-gray-100">
          ...
          <button id="admin-gallery-apply" type="button" class="ml-auto px-3 py-1.5 rounded bg-indigo-600 hover:bg-indigo-700 text-white text-sm">Apply</button>
          <button id="admin-gallery-refresh" type="button" class="px-3 py-1.5 rounded bg-gray-200 dark:bg-gray-800 hover:bg-gray-300 dark:hover:bg-gray-700 text-sm">Refresh</button>
        </section>

        <!-- Stats strip for the current filter -->
        <section id="admin-gallery-stats" class="hidden flex-wrap gap-x-5 gap-y-1 text-xs text-gray-500 dark:text-gray-400
                                   ${AdminUI.card} px-3 py-2"></section>

        <div id="admin-gallery-proposals" class="space-y-4"></div>

        <div class="flex justify-center py-4">
          <button id="admin-gallery-load-older" type="button" class="hidden px-4 py-2 rounded bg-gray-200 dark:bg-gray-800 hover:bg-gray-300 dark:hover:bg-gray-700 text-sm">Load older</button>
          <span id="admin-gallery-empty" class="hidden text-gray-500 text-sm">No merged proposals match these filters yet.</span>
        </div>
      </main>
    </div>`;
...
if (typeof window !== 'undefined') window.AdminGallery = AdminGallery;
