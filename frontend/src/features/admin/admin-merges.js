...
const AdminMerges = (() => {
  ...
  function runCard(run) {
    ...
    el.innerHTML = `
      <button type="button" class="run-head w-full text-left px-4 py-3 flex items-center gap-3 hover:bg-zinc-100 dark:hover:bg-zinc-800/60">
        <span class="chev text-zinc-500 dark:text-zinc-400 transition-transform">▶</span>
    ...
    head.addEventListener('click', async () => {
      ...
      if (!loaded) {
        body.innerHTML = '<div class="text-xs text-zinc-500 dark:text-zinc-400">Loading steps…</div>';
        try {
  ...
  function stepsHtml(steps) {
    if (!steps.length) return '<div class="text-xs text-zinc-500 dark:text-zinc-400">No steps recorded.</div>';
        ...
        <div class="flex items-start gap-2">
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-baseline gap-2">
              <span class="text-[10px] uppercase tracking-wide text-zinc-500 dark:text-zinc-400 font-mono">${esc(s.phase || '')}</span>
            ...
            ${hasDetail ? `<button type="button" class="detail-toggle text-[11px] text-violet-500 dark:text-violet-400 hover:text-violet-800 dark:hover:text-violet-300 dark:hover:text-violet-300 mt-0.5">detail</button>
  ...
  const MARKUP = `
    <div id="admin-merges-root">
      ...
      <div id="admin-merges-gate" class="hidden text-zinc-500 dark:text-zinc-400 text-center py-20"></div>
      ...
      <main id="admin-merges-content" class="hidden space-y-4">
        ...
        <div class="flex justify-center py-4">
          ...
          <span id="admin-merges-empty" class="hidden text-zinc-500 dark:text-zinc-400 text-sm">No merge runs match these filters yet.</span>
        </div>
      </main>
    </div>`;
