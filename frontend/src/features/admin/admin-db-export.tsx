    ...
    <div id="admin-db-export-panel" className="space-y-4">
      <div className="rounded-lg border border-red-300 dark:border-red-900 bg-red-50 dark:bg-red-950/40 p-4">
        ...
        <p className="text-sm text-red-800 dark:text-red-200 mt-3">
          ...
          {' contain the individual apps’ own databases, uploaded app-file bytes (those live in object storage), the chain node’s data, or the platform’s environment file. That matters, because the key that decrypts the API-key and app-secret blobs lives only there.'}
        </p>
      </div>
