...
const AdminTopochain = {
    ...
    if (hero) {
      if (ok && data?.success) {
        ...
        hero.innerHTML = `<section class="${PANEL_CLS} px-4 py-4 sm:px-5">
          ...
          <p class="mt-0.5 text-xs text-zinc-500 dark:text-zinc-400">${esc(AdminTopochain._fmt(ev.starts_at))} &ndash; ${esc(AdminTopochain._fmt(ev.ends_at))}</p>
  ...
  async _moveChallenge(eventId, challengeId) {
    ...
    if (!others.length) {
      ...
      host.innerHTML = `<div class="mt-3">${AdminTopochain._panel({
        ...
        body: '<p class="text-sm text-zinc-500 dark:text-zinc-400">There is no other event to move this challenge to.</p>',
      })}</div>`;
    ...
    table.innerHTML = AdminTopochain._list({
      ...
      extra: (w) => (w.answers ? `
        <details class="text-xs">
          <summary class="cursor-pointer select-none text-zinc-500 dark:text-zinc-400 min-h-[36px] flex items-center">Survey answers</summary>
      ...
      columns: [
        {
          ...
          cell: (a) => `<button data-acct-show="${a.id}" type="button"
            class="text-left break-all underline decoration-dotted underline-offset-2 hover:text-violet-600 dark:hover:text-violet-400 rounded touch-manipulation focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-500"
        ...
        {
          label: 'Delegation',
          cell: (a) => (a.delegated ? AdminTopochain._badgeHtml('Delegated', 'green') : '<span class="text-zinc-400">—</span>'),
        },
  ...
  _renderAccountDetail(a) {
    ...
    const row = (label, valueHtml) => `<div class="py-2">
      <dt class="text-xs uppercase tracking-wide text-zinc-500 dark:text-zinc-400">${esc(label)}</dt>
    ...
    const secretHtml = secret != null
      ...
      : '<span class="text-zinc-500 dark:text-zinc-400">Hidden for view-only admins.</span>';
    const userHtml = a.user
      ...
      : '<p class="text-sm text-zinc-500 dark:text-zinc-400">Unassigned — no user has claimed this account.</p>';
    body.innerHTML = `
      <dl class="divide-y divide-zinc-100 dark:divide-zinc-800">
    ...
    ? `${AdminTopochain._badgeHtml('Delegated', 'green')} <span class="text-xs text-zinc-500 dark:text-zinc-400">since ${esc(AdminTopochain._fmt(a.delegated_since))}</span>`
    : '<span class="text-zinc-500 dark:text-zinc-400">Not delegated</span>')}
  ...
  async _openAccountImportForm() {
    ...
    'registration_code is generated server-side; do not include it.')}
        ...
        <div id="admin-topo-oa-imp-result" class="mt-3 text-xs text-zinc-500 dark:text-zinc-400"></div>`,
  ...
  async _openActivityImportForm() {
    ...
    '[\n  {"user_id":1,"season_event_id":1,"challenge_id":1,"activity_type":"community_contribution","points":10,"activity_at":"2026-01-01T00:00:00.000Z"}\n]', 8))}
        ...
        <div id="admin-topo-act-imp-result" class="mt-3 text-xs text-zinc-500 dark:text-zinc-400"></div>`,
      ...
      body.innerHTML = `
        ...
        <div class="mt-4 grid grid-cols-1 gap-5 md:grid-cols-2">
          <div class="min-w-0 overflow-x-auto">
            ...
            <table class="w-full"><thead class="text-xs text-zinc-500 dark:text-zinc-400"><tr><th class="text-left px-2">User</th><th class="text-right px-2">Points</th><th class="text-right px-2">Count</th></tr></thead><tbody>${userRows}</tbody></table></div>
          <div class="min-w-0 overflow-x-auto">
            <div class="text-xs font-semibold uppercase tracking-wide text-zinc-400 dark:text-zinc-500 mb-1">By type</div>
            <table class="w-full"><thead class="text-xs text-zinc-500 dark:text-zinc-400"><tr><th class="text-left px-2">Type</th><th class="text-right px-2">Count</th><th class="text-right px-2">Points</th><th class="text-right px-2">Users</th></tr></thead><tbody>${typeRows}</tbody></table></div>
        </div>`;
    };
  ...
  _renderDelegationStats() {
    ...
    const tile = (label, value, valueCls) => `<div class="${PANEL_CLS} px-4 py-3">
      <p class="text-xs uppercase tracking-wide text-zinc-500 dark:text-zinc-400">${esc(label)}</p>
  ...
  _renderDelegationsTable() {
    ...
    table.innerHTML = AdminTopochain._list({
      ...
      columns: [
        ...
        { label: 'Since', cell: (d) => esc(AdminTopochain._fmt(d.started_at)), tdClass: 'text-xs text-zinc-500' },
        { label: 'Ended', cell: (d) => esc(AdminTopochain._fmt(d.ended_at)), tdClass: 'text-xs text-zinc-500' },
        { label: 'Periods', cell: (d) => esc(d.period_count), tdClass: 'tabular-nums text-right text-xs text-zinc-500', thClass: 'text-right' },
      ],
  ...
  _dlgHistoryHtml() {
    ...
    if (!s.history.length) {
      return '<p class="text-xs text-zinc-500 dark:text-zinc-400">No periods recorded for this account.</p>';
    }
    const rows = s.history.map((p) => `<li class="flex flex-wrap items-center gap-2 py-1.5">
      ...
      <span class="text-xs text-zinc-600 dark:text-zinc-300">${esc(AdminTopochain._fmt(p.started_at))}</span>
      <span class="text-xs text-zinc-400">&rarr;</span>
      <span class="text-xs text-zinc-600 dark:text-zinc-300">${p.ended_at ? esc(AdminTopochain._fmt(p.ended_at)) : 'now'}</span>
    </li>`).join('');
    return `<div>
      <p class="text-xs font-medium uppercase tracking-wide text-zinc-500 dark:text-zinc-400">Delegation history, newest first</p>
      <ul class="mt-1 divide-y divide-zinc-100 dark:divide-zinc-800">${rows}</ul>
    </div>`;
  },
  ...
  _dlgPartyChip({ party, avatar, titleHtml, subHtml }) {
    return `<span class="inline-flex items-start gap-2 text-left" data-dlg-party="${party}">
      ...
      <span class="inline-flex min-w-0 flex-col">
        ...
        ${subHtml ? `<span class="text-[11px] leading-4 text-zinc-500 dark:text-zinc-400">${subHtml}</span>` : ''}
      </span>
    </span>`;
  },
  ...
  _dlgAvatarHtml(u) {
    const esc = AdminTopochain.esc;
    if (u && u.avatar_url) {
      return `<img src="${esc(u.avatar_url)}" alt="" class="mt-0.5 h-6 w-6 shrink-0 rounded-full bg-zinc-100 object-cover dark:bg-zinc-800">`;
    }
  ...
  _dlgDelegatorHtml(d) {
    ...
    if (u) {
      ...
      return AdminTopochain._dlgPartyChip({
        ...
        titleHtml: `${esc(name)} <span class="text-xs font-normal text-zinc-500 dark:text-zinc-400">user #${esc(u.user_id)}</span>`,
        subHtml: address,
      });
    }
    ...
    return AdminTopochain._dlgPartyChip({
      ...
      titleHtml: `<span class="font-normal text-zinc-500 dark:text-zinc-400">${d.onchain_account_id != null ? 'Unclaimed account' : 'Account not on file'}</span>`,
      subHtml: address,
    });
  },
  ...
  _dlgDelegateeHtml() {
    return AdminTopochain._dlgPartyChip({
      party: 'delegatee',
      avatar: `<span class="mt-0.5 inline-flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-violet-600 text-[11px] font-semibold text-white">P</span>`,
    ...
    if (m.configured) {
      host.innerHTML = `
        <div class="${PANEL_CLS} px-4 py-3 text-sm sm:px-5">
          ...
          <span class="text-zinc-500 dark:text-zinc-400"> — login codes and waitlist confirmations are being sent
  ...
  async _loadAppVersionActivity() {
    ...
    host.innerHTML = `
      <h3 class="text-sm font-semibold mt-8 mb-3">
        ...
        <span class="font-normal text-zinc-500 dark:text-zinc-400">(${esc(a.total)} total)</span>
      </h3>
