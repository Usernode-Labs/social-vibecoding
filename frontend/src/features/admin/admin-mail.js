...
const AdminMail = (() => {
  ...
  async function loadStatus() {
    ...
    if (!ok || !data) {
      el.innerHTML = '<p class="text-sm text-zinc-500 dark:text-zinc-400">Could not load the mail configuration.</p>';
      return;
    }
  ...
  function renderOutcome(outcome) {
    ...
    const rows = [
      ...
      <div class="flex gap-2">
        <span class="text-zinc-500 dark:text-zinc-400 w-24 shrink-0">${esc(k)}</span>
    ...
    el.innerHTML = `
      <div class="${AdminUI.card} px-4 py-3 text-sm mt-3">
        ...
        ${subject ? `<div class="text-xs text-zinc-500 dark:text-zinc-400 mt-2">Subject: ${esc(subject)}</div>` : ''}
      </div>`;
  }
  ...
  async function sendTest() {
    ...
    if (el) {
      el.innerHTML = '<p class="text-sm text-zinc-500 dark:text-zinc-400 mt-3">Sending&hellip; this can take up to about 17 seconds if the provider is slow.</p>';
    }
  ...
  async function loadActivity() {
    ...
    if (!ok || !data) {
      el.innerHTML = '<p class="text-sm text-zinc-500 dark:text-zinc-400">Could not load recent activity.</p>';
      return;
    }
  ...
  highlightId && r.id === highlightId ? ' bg-violet-50 dark:bg-violet-950/40' : ''}">
        <td class="py-1.5 pr-3 whitespace-nowrap text-zinc-500 dark:text-zinc-400">${esc(
    r.created_at ? String(r.created_at).replace('T', ' ').slice(0, 19) : '')}</td>
        ...
        <td class="py-1.5 pr-3 whitespace-nowrap text-zinc-500 dark:text-zinc-400">${esc(r.provider || '—')}</td>
        <td class="py-1.5 pr-3 whitespace-nowrap font-medium ${
  statusClass(r.status)}">${esc(r.status || '')}</td>
        <td class="py-1.5 text-zinc-500 dark:text-zinc-400">${esc(r.error || '')}</td>
      </tr>`).join('');
      ...
      <div class="rounded-lg border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 px-4 py-3">
        <div class="flex flex-wrap items-baseline justify-between gap-2">
          ...
          <div class="flex items-center gap-2">
            <span class="text-xs text-zinc-500 dark:text-zinc-400">${totals ? `last 24h: ${totals}` : 'nothing in the last 24h'}</span>
  ...
  return {
    render(hostEl) {
      ...
      const form = canWrite() ? `
        <div class="flex flex-wrap items-end gap-2">
          <label class="block text-xs grow max-w-md">
            <span class="text-zinc-500 dark:text-zinc-400">Recipient</span>
      ...
      host.innerHTML = `
        ...
        <div id="admin-mail-status" class="mb-4"><p class="text-sm text-zinc-500 dark:text-zinc-400">Loading&hellip;</p></div>
        ...
        <div id="admin-mail-activity"><p class="text-sm text-zinc-500 dark:text-zinc-400">Loading&hellip;</p></div>`;
