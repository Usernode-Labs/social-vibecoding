    ...
    <div ref={dialog.rootRef} id="messages-share-dialog" className="hidden fixed inset-0 z-50 overflow-y-auto overscroll-contain bg-black/60" {...dialog.backdropProps}>
      <div data-modal-backdrop="" className="flex min-h-full items-center justify-center p-4">
        <div className="bg-white dark:bg-zinc-900 rounded-xl p-5 w-full max-w-md shadow-xl">
          <div className="flex items-center justify-between mb-4"><div><h2 className="text-lg font-bold">Share item</h2><p className="text-xs text-zinc-500">Access is checked separately for every recipient.</p></div><button type="button" onClick={dialog.close} className="text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-200" aria-label="Close">×</button></div>
          <label className="block mb-3"><span className="block text-xs font-medium text-zinc-500 mb-1">Item type</span><select value={type} onChange={(event) => { setType(event.target.value as SharedObjectType); setItemId(''); setVersion(''); }} className="w-full rounded-lg bg-zinc-100 dark:bg-zinc-800 border border-zinc-300 dark:border-zinc-700 px-3 py-2 text-sm text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-violet-500"><option value="app">App</option><option value="issue">GitHub-backed issue</option><option value="proposal">Code proposal</option><option value="governance">Governance proposal</option><option value="spec">Exact spec version</option></select></label>
          <label className="block mb-3"><span className="block text-xs font-medium text-zinc-500 mb-1">App</span><select value={appId || ''} disabled={loading} onChange={(event) => setAppId(api.strictId(event.target.value))} className="w-full rounded-lg bg-zinc-100 dark:bg-zinc-800 border border-zinc-300 dark:border-zinc-700 px-3 py-2 text-sm text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-violet-500"><option value="">{loading ? 'Loading apps…' : 'Choose an app'}</option>{apps.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>
          {type !== 'app' ? <label className="block mb-3"><span className="block text-xs font-medium text-zinc-500 mb-1">{type === 'issue' ? 'Issue number' : type === 'governance' ? 'Governance proposal ID' : 'Proposal / session ID'}</span><Input inputMode="numeric" pattern="[0-9]*" value={itemId} onChange={(event) => setItemId(event.target.value.replace(/\D/g, '').slice(0, 10))} placeholder="123" /></label> : null}
