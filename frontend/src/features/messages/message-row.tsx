  ...
  return (
    <article id={`messages-message-${message.id}`} data-message-id={message.id} className={`messages-message group ${mine ? 'messages-message-self' : ''} ${message.saved ? 'messages-message-saved' : ''} ${message.pending ? 'messages-message-pending' : ''} ${message.failed ? 'messages-message-failed' : ''}`} onPointerDown={startLongPress} onPointerUp={cancelLongPress} onPointerCancel={cancelLongPress} onPointerMove={cancelLongPress}>
      ...
      <div className="min-w-0 flex-1">
        <div className="messages-message-head"><span className={mine ? 'text-azure-800 dark:text-azure-200' : ''}>@{message.sender.username}</span><time title={fullTime(message.createdAt)}>{new Date(message.createdAt).toLocaleTimeString(undefined, { hour: 'numeric', minute: '2-digit' })}</time>{message.editedAt ? <span title={fullTime(message.editedAt)}>edited</span> : null}{message.pending ? <span>sending…</span> : null}{message.failed ? <span className="text-red-700 dark:text-red-200">not sent</span> : null}</div>
        ...
        {reporting ? <form className="messages-report" onSubmit={(event) => { event.preventDefault(); void report(); }}>
          ...
          <label><span>Details <span className="font-normal text-zinc-500 dark:text-zinc-300">(optional)</span></span><textarea value={reportDetail} onChange={(event) => setReportDetail(event.target.value.slice(0, 500))} rows={2} maxLength={500} /></label>
        ...
        {notice ? <p role="status" className={`mt-1 text-[11px] ${notice === 'Report submitted.' ? 'text-meadow-700 dark:text-meadow-200' : 'text-red-700 dark:text-red-200'}`}>{notice}</p> : null}
      </div>
