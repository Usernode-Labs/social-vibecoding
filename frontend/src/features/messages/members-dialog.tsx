      ...
      <div data-modal-backdrop="" className="flex min-h-full items-center justify-center p-4">
        <div className="bg-white dark:bg-zinc-900 rounded-xl p-5 w-full max-w-md shadow-xl dark:shadow-none">
          <div className="flex items-center justify-between mb-4">
            <div><h2 className="text-lg font-bold">Group members</h2><p className="text-xs text-zinc-500 dark:text-zinc-300">Accepted members can read the complete retained history.</p></div>
          ...
          <div className="max-h-56 overflow-y-auto divide-y divide-zinc-100 dark:divide-zinc-800">
            {active?.members.map((member) => (
              <div key={member.id} className="flex items-center gap-3 py-2">
                ...
                <div className="min-w-0"><div className="text-sm font-medium truncate">@{member.username}</div><div className="text-[11px] text-zinc-500 dark:text-zinc-300 capitalize">{member.role}{member.status !== 'member' ? ` · ${member.status}` : ''}</div></div>
                {active.canManage && member.role !== 'owner' && member.status === 'member' ? <button type="button" disabled={busy} onClick={() => void remove(member)} className="ml-auto text-xs text-red-700 dark:text-red-200 disabled:opacity-50">Remove</button> : null}
              </div>
            ))}
          </div>
          {active?.canInvite ? (
            <div className="mt-4 pt-4 border-t border-zinc-200 dark:border-zinc-800">
              <label className="block text-xs font-medium text-zinc-500 dark:text-zinc-300 mb-1">Invite people</label>
              {selected.length ? <div className="flex flex-wrap gap-1 mb-2">{selected.map((user) => <button type="button" key={user.id} onClick={() => setSelected((current) => current.filter((item) => item.id !== user.id))} className="rounded-full bg-azure-100 dark:bg-azure-950 text-azure-700 dark:text-azure-300 px-2 py-1 text-xs">@{user.username} ×</button>)}</div> : null}
              <Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search by username" autoComplete="off" />
              {query.trim() ? <div className="mt-1 max-h-32 overflow-y-auto">{available.map((user) => <button type="button" key={user.id} onClick={() => { setSelected((current) => [...current, user]); setQuery(''); }} className="w-full flex items-center gap-2 py-1.5 px-1 text-sm hover:bg-zinc-50 dark:hover:bg-zinc-800 rounded"><UserAvatar user={user} size="sm" />@{user.username}<span className="ml-auto text-xs text-azure-800 dark:text-azure-200">Add</span></button>)}</div> : null}
              {/* `layout="stacked3"`, not a hand-written `mt-3 w-full`: the
                  12px lead-in under the selected-people row is a named value
                  in button.tsx's layout table — which names THIS button
                  (#members-invite-confirm) as the site it exists for — and a
                  margin spelled at the call site is the caller reaching into
                  the callee's layout. */}
              <Button type="button" layout="stacked3" disabled={busy || !selected.length} onClick={() => void invite()}>{busy ? 'Inviting…' : `Invite ${selected.length || ''}`.trim()}</Button>
            </div>
          ) : null}
          ...
          <div className="mt-4 pt-4 border-t border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
            <button type="button" disabled={busy} onClick={() => void leaveCurrent()} className="text-xs text-red-700 dark:text-red-200 disabled:opacity-50">Leave group</button>
