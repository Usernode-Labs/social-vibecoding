...
function SetupStep({ n, title, children }: {
  ...
  return (
    <li className="flex gap-2.5">
      ...
      <span className="min-w-0 text-xs text-zinc-500 dark:text-zinc-300 leading-relaxed">
        ...
        <strong className="font-semibold text-zinc-600 dark:text-zinc-300">{`${title} `}</strong>
        {children}
      </span>
    </li>
  );
  ...
  return (
    <div data-settings-section="connectors" className="hidden">
      <div id="connectors-section">
        ...
        <Label spacing="stacked" htmlFor="connector-url">
          Connector URL
        </Label>
        ...
        <div className="mb-2 rounded-md border border-zinc-200 dark:border-zinc-800 p-3">
          ...
          <ol className="space-y-2">
            <SetupStep n={1} title="Open connector settings.">
              Go to <strong className="font-semibold text-zinc-600 dark:text-zinc-300">Customize &rarr; Connectors</strong> in Claude (<code className="font-mono text-zinc-600 dark:text-zinc-300">claude.ai/customize/connectors</code>). This is where both directory connectors and your own custom ones live.
            </SetupStep>
            <SetupStep n={2} title="Start a custom connector.">
              Click the <code className="font-mono text-zinc-600 dark:text-zinc-300">+</code> button, then choose &ldquo;Add custom connector&rdquo;. On Team or Enterprise plans this option isn&rsquo;t there for members, so an Owner adds it first from Organization settings &rarr; Connectors (Add &rarr; hover &ldquo;Custom&rdquo; &rarr; &ldquo;Web&rdquo;).
            </SetupStep>
            <SetupStep n={3} title="Paste your MCP server URL.">
              For Usernode that is the connector URL in the field above, a public HTTPS endpoint ending in <code className="font-mono text-zinc-600 dark:text-zinc-300">/mcp</code>. A custom server must be reachable from Anthropic&rsquo;s cloud, not just from your machine.
            </SetupStep>
            ...
            <SetupStep n={6} title="Enable it in a conversation.">
              In a chat, use the <code className="font-mono text-zinc-600 dark:text-zinc-300">+</code> button at the lower left, then &ldquo;Connectors&rdquo;, and toggle your connector on. Toggles are per-conversation, so you control which chats can reach it.
            </SetupStep>
          </ol>
        </div>
        <div className="mb-2 rounded-md border border-zinc-200 dark:border-zinc-800 p-3">
          ...
          <ol className="space-y-2">
            ...
            <SetupStep n={2} title="Turn on Developer mode.">
              In ChatGPT, go to <strong className="font-semibold text-zinc-600 dark:text-zinc-300">Profile &rarr; Settings &rarr; Security and login &rarr; Developer mode</strong> and turn Developer mode on.
            </SetupStep>
            <SetupStep n={3} title="Open the ChatGPT Plugins page.">
              After Developer mode is enabled, open <strong className="font-semibold text-zinc-600 dark:text-zinc-300">ChatGPT Plugins</strong>.
            </SetupStep>
            <SetupStep n={4} title="Click the + button.">
              The <code className="font-mono text-zinc-600 dark:text-zinc-300">+</code> button lets you add your own MCP-backed app.
            </SetupStep>
            <SetupStep n={5} title="Enter your MCP server details.">
              Enter the URL of the remote MCP server (for Usernode, the connector URL in the field above) and configure authentication if required. The server must be reachable by ChatGPT; one running only on <code className="font-mono text-zinc-600 dark:text-zinc-300">localhost</code> will not work directly.
            </SetupStep>
            ...
            <SetupStep n={7} title="Use the MCP server in a chat.">
              Start a <strong className="font-semibold text-zinc-600 dark:text-zinc-300">new ChatGPT conversation</strong> and open the <code className="font-mono text-zinc-600 dark:text-zinc-300">+</code> / tools menu next to the message box. Select Developer mode, then select the MCP app you just created. Now ask ChatGPT to perform something that uses one of the tools, for example: <em>&ldquo;Use my MCP server to list the open support tickets.&rdquo;</em> When appropriate, ChatGPT will call the tools your MCP server exposes and use their results in the conversation.
            </SetupStep>
          </ol>
          <p className="mt-3 pt-2 border-t border-zinc-200 dark:border-zinc-800 text-xs text-zinc-500 dark:text-zinc-300 leading-relaxed">
            <strong className="font-semibold text-zinc-600 dark:text-zinc-300">In short:</strong> Settings &rarr; Security and login &rarr; Developer mode ON &rarr; ChatGPT Plugins &rarr; <code className="font-mono text-zinc-600 dark:text-zinc-300">+</code> &rarr; Enter MCP server URL &rarr; Create &rarr; New chat &rarr; <code className="font-mono text-zinc-600 dark:text-zinc-300">+</code> &rarr; Developer mode &rarr; select your MCP app &rarr; Ask ChatGPT to use it.
          </p>
        </div>
        <p className="text-xs text-zinc-500 dark:text-zinc-300 mb-2 leading-relaxed">
        ...
        */}
        <p className="text-xs text-zinc-500 dark:text-zinc-300 mb-4 leading-relaxed">
          Name it exactly <code className="font-mono text-zinc-600 dark:text-zinc-300">usernode</code>. Claude Code builds its permission rules from that name. A different spelling still works, but the read-only allowlist Usernode ships in every app repo will not match it, and you will keep being asked to approve each call. The allowlist covers <code className="font-mono text-zinc-600 dark:text-zinc-300">Usernode</code> as well, so the capitalised form is safe; anything else needs the rules rewritten, which the field further down does for you.
        </p>
        {/*
        ...
        */}
        <div id="connector-prompt-help" className="mb-4 rounded-md border border-zinc-200 dark:border-zinc-800 p-3">
          ...
          <p className="text-xs text-zinc-500 dark:text-zinc-300 mb-3 leading-relaxed">
            Claude Code asks you to approve <em>every</em> connector call by default, including read-only ones like <code className="font-mono text-zinc-600 dark:text-zinc-300">whoami</code> and <code className="font-mono text-zinc-600 dark:text-zinc-300">get_app</code>. Which fix applies depends on where you run it.
          </p>
          {/*
          ...
          */}
          ...
          <div id="connector-case-cc-local" className="mb-3">
            <h5 className="text-xs font-semibold text-zinc-600 dark:text-zinc-300 mb-1">
              Claude Code on your own machine
            </h5>
            <p className="text-xs text-zinc-500 dark:text-zinc-300 mb-2 leading-relaxed">
              Add this to your own <code className="font-mono text-zinc-600 dark:text-zinc-300">~/.claude/settings.json</code>. It is the only one of the three that covers <strong className="font-semibold text-zinc-600 dark:text-zinc-300">every</strong> repo at once, including repos Usernode never made.
            </p>
            {/*
            ...
            */}
            <div className="flex items-center justify-between gap-2 mb-1">
              <span className="font-mono text-[11px] text-zinc-500 dark:text-zinc-300 truncate">~/.claude/settings.json</span>
              <Button
            ...
            <pre id="connector-allow-rules" className="min-w-0 overflow-x-auto text-xs font-mono text-zinc-600 dark:text-zinc-300 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-md p-2">{PERSONAL_ALLOW_RULES}</pre>
          </div>
          ...
          <div id="connector-case-cc-web" className="mb-3">
            ...
            <pre id="connector-repo-allow-rules" className="min-w-0 overflow-x-auto text-xs font-mono text-zinc-600 dark:text-zinc-300 bg-zinc-50 dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-md p-2 mb-2">{PERSONAL_ALLOW_RULES}</pre>
          ...
          */}
          <div className="mt-3">
            <Label spacing="stacked" htmlFor="connector-name-spelling">
              Connector registered under a different name?
            </Label>
            <p className="text-xs text-zinc-500 dark:text-zinc-300 mb-2 leading-relaxed">
              Check what your tools are called in your session, the middle part of <code className="font-mono text-zinc-600 dark:text-zinc-300">mcp__usernode__whoami</code>. If it is not <code className="font-mono text-zinc-600 dark:text-zinc-300">usernode</code> or <code className="font-mono text-zinc-600 dark:text-zinc-300">Usernode</code>, type it here and both blocks above are rewritten for it.
            </p>
            <Input
          ...
          */}
          <p id="connector-hint-status" className="hidden text-xs text-zinc-500 dark:text-zinc-300 mt-2 leading-relaxed"></p>
        </div>
        <h4 className="text-xs font-semibold text-zinc-700 dark:text-zinc-300 mb-2">
          Connected
        </h4>
      ...
      */}
      <div id="dev-flow-pref-section" className="mt-6 pt-6 border-t border-zinc-200 dark:border-zinc-800">
        ...
        <p className="text-xs text-zinc-500 dark:text-zinc-300 mb-3 leading-relaxed">
          ...
          <strong className="font-semibold text-zinc-600 dark:text-zinc-300">Ask me every time</strong>
        ...
        */}
        <select
          ...
          className="w-full rounded-lg bg-zinc-100 dark:bg-zinc-800 border border-zinc-300 dark:border-zinc-700 px-3 py-2 text-sm text-zinc-900 dark:text-zinc-100 focus:outline-none focus:ring-2 focus:ring-zinc-900 dark:focus:ring-zinc-100"
          defaultValue=""
        >
