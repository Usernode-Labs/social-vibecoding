...
for (const scenario of ['current', 'stale', 'missing-label', 'missing-deployment', 'api-error', 'old-commit', 'no-config']) {
  test(`Kubernetes preview freshness: ${scenario}`, async (t) => {
    ...
    kubernetes._setClientsForTest({ apps: { readNamespacedDeployment: async ({ name, namespace }) => {
      ...
      return { status: { availableReplicas: 1 }, spec: { template: { metadata: { labels } } } };
    } } });
    const rebuild = await stagingNeedsRebuild(session, {
      config: scenario === 'no-config' ? null : config,
      headSha: scenario === 'old-commit' ? 'def' : 'abc',
    });
    assert.equal(rebuild, ['stale', 'missing-label', 'missing-deployment', 'old-commit'].includes(scenario));
  });
}
