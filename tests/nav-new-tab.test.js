...
test('"back out of an issue / proposal / governance topic" rides the header anchor', () => {
  ...
  assert.match(read('frontend/src/features/header/platform-header.tsx'),
    /subTab === 'topic'\) return boardHref\(slug, boardView\);/,
    'the header points a topic page at its board — `boardHref` and not a '
    + 'literal `/board`, because Workshop and Board are one screen in two '
    + 'layouts and the arrow has to name the one the reader came from');
});
