...
test('staging ?demo=1 attaches synthetic headless to mocks 900003/900005 only', async () => {
  ...
  try {
    ...
    // 5 live issues + 11 appended mocks (900008 joined in #556, 900009 in
    // #617, 900010 in #683, 900011 with the card-as-pointer revision — the
    // deliberately BARE row, which the staging attribute-enrichment block
    // leaves alone so the 'no grey placeholder chips' rule is reviewable).
    assert.strictEqual(body.issues.length, 16);
