...
test('the head declares color-scheme for both grounds', () => {
  ...
  assert.match(
    src,
    /html\s*\{[^}]*background-color:\s*#eee8d5[^}]*color-scheme:\s*light[^}]*\}/,
...
test('color-scheme is keyed off .dark, never delegated to the OS', () => {
  ...
  // #eee8d5 page. This is the regression the rule above prevents.
  assert.doesNotMatch(
    src,
...
test('the resolved appearance is read back off the document', async () => {
  ...
  const light = loadNativeChrome({ dark: false, background: 'rgb(238, 232, 213)' });
  await light.ready;
  assert.deepEqual(plain(light.calls.setAppearance), [
    { scheme: 'light', background: '#eee8d5' },
  ]);
});

test('it publishes on boot and on every theme change', async () => {
  const harness = loadNativeChrome({ dark: false, background: 'rgb(238, 232, 213)' });
