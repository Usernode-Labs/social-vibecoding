...
// ═══════════════════════════════════════════════════════════════════════
// 6. The dev-session branch charset
// ═══════════════════════════════════════════════════════════════════════

test('every handle this validator accepts is a pushable dev-session branch', () => {
  // Dev sessions mint `dev/<username>-<timestamp>` and the push proxy
  // validates the result (src/services/branch-names.js). #1377 is the bug
  // where a username outside that charset stranded 47 minutes of agent work
  // behind `bad_branch` — reached by REGISTERING such a name. A rename is a
  // second door to the same failure, and the only thing holding it shut is
  // that this validator's charset is a SUBSET of the branch one. Nothing
  // else would notice if somebody widened it.
  const branchNames = require('../src/services/branch-names');
  const candidates = [
    'ada', 'Ada99', 'ada_lovelace', '___', 'a'.repeat(32),
    'A1_b2_C3', '000', 'z_9',
  ];
  for (const name of candidates) {
    assert.equal(usernames.validateUsername(name).ok, true, `${name} should validate`);
    assert.equal(
      branchNames.isValidBranchName(branchNames.devBranchName(name)), true,
      `a renamed @${name} must still be able to push a dev session`,
    );
  }
});

test('the username charset is a strict subset of the branch charset', () => {
  // Character-by-character, so this fails on a widening rather than on
  // whichever example someone happened to add above.
  const branchSrc = read('src/services/branch-names.js');
  const m = branchSrc.match(/BRANCH_SAFE_CHARS_RE\s*=\s*\/\^\[([^\]]+)\]/);
  assert.ok(m, 'branch-names.js must still expose a BRANCH_SAFE_CHARS_RE character class');
  const branchClass = new RegExp(`^[${m[1]}]$`);
  for (const ch of 'abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_') {
    assert.equal(usernames.validateUsername(`aa${ch}`).ok, true, `${ch} accepted by usernames`);
    assert.equal(branchClass.test(ch), true,
      `'${ch}' is accepted in a username but not in a branch name — a rename into it `
