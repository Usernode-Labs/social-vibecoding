...
test('Social negotiates walletless issuance and exact replay with the existing protocol-2 client', async (t) => {
  const { NativeSessionProtocol } = require('../src/services/topochain/native-session-protocol');
  const { sha256Hex, validateInstallationKeys, possessionTranscript } = require('../src/services/topochain/native-session-crypto');
  const { compactDecrypt, importJWK } = await import('jose');
  const possession = crypto.generateKeyPairSync('ec', { namedCurve: 'P-256' });
  const envelope = crypto.generateKeyPairSync('rsa', { modulusLength: 3072, publicExponent: 0x10001 });
  const privateKey = await importJWK(envelope.privateKey.export({ format: 'jwk' }), 'RSA-OAEP');
  const network = { id: 'testnet', chainId };

  await withDatabase(t, async (pool) => {
    await pool.query(`
      ALTER TABLE users ADD COLUMN bp_released_at TIMESTAMPTZ;
      ALTER TABLE mobile_auth_tokens ALTER COLUMN id ADD GENERATED BY DEFAULT AS IDENTITY;
      ALTER TABLE onchain_accounts
        ADD COLUMN season_id BIGINT,
        ADD COLUMN season_event_id BIGINT,
      ...
      CREATE TABLE user_enrollments (
        user_id BIGINT, season_id BIGINT, season_event_id BIGINT,
        registered_at TIMESTAMPTZ, created_at TIMESTAMPTZ, updated_at TIMESTAMPTZ
      );
    `);
    await pool.query(extractTable('native_session_credential_envelopes'));
    const protocol = new NativeSessionProtocol({ pool, config: {
      nativeSessionV2Network: network, dataEncryptionKey: 'walletless-compat-test-key',
    } });

    // Exercise the actual upgrade statement on a populated pre-change table,
    // then run it again as the application does on subsequent starts.
    await pool.query("INSERT INTO users (id, username) VALUES (99, 'migration-fixture')");
    await pool.query('INSERT INTO native_session_web_incarnations (id, user_id) VALUES ($1, 99)', [opaque('nsw_', 99)]);
    await pool.query(`INSERT INTO native_session_attempts
      (attempt_id, user_id, web_session_incarnation_id, desired_runtime, network_id, chain_id, request_digest)
      VALUES ($1, 99, $2, 'running', 'testnet', $3, $4)`,
    [opaque('nsa_', 99), opaque('nsw_', 99), chainId, 'ab'.repeat(32)]);
    await pool.query('ALTER TABLE native_session_attempts DROP COLUMN walletless_supported');
    const migration = schemaSql.match(/ALTER TABLE native_session_attempts\s+ADD COLUMN IF NOT EXISTS walletless_supported BOOLEAN NOT NULL DEFAULT FALSE;/)?.[0];
    assert.ok(migration, 'production includes an additive compatibility migration');
    await pool.query(migration);
    await pool.query(migration);
    assert.equal((await pool.query('SELECT walletless_supported FROM native_session_attempts')).rows[0].walletless_supported, false);

    let userId = 100;
    for (const wallet of ['no-season', 'no-account', 'existing-account']) {
      await pool.query('DELETE FROM seasons');
      if (wallet !== 'no-season') {
        await pool.query(`INSERT INTO seasons VALUES
          (7, FALSE, TRUE, NOW() - INTERVAL '1 day', NOW() + INTERVAL '1 day')`);
      }
      for (const [appVersion, buildNumber, compatible] of [
        ['0.4.0', undefined, false], ['0.4.0', '1250', false], ['0.4.0', '1252', true],
        ['0.4.1', '1', true], ['0.5.0', '1', true], ['1.0.0', '1', true],
      ]) {
        await t.test(`${wallet}: ${appVersion}+${buildNumber || 'unknown'}`, async () => {
          userId += 1;
          const sessionToken = `session-${userId}`;
          await pool.query('INSERT INTO users (id, username) VALUES ($1, $2)', [userId, `user-${userId}`]);
          await pool.query(`INSERT INTO sessions (token, user_id, expires_at)
            VALUES ($1, $2, NOW() + INTERVAL '1 day')`, [sessionToken, userId]);
          if (wallet === 'existing-account') {
            await pool.query(`INSERT INTO onchain_accounts
              (id, user_id, address, season_id, public_key, secret_key)
              VALUES ($1, $1, $2, 7, 'public-key', 'secret-key')`, [userId, `ut1-${userId}`]);
          }
          const body = { protocol: 2, attemptId: opaque('nsa_', userId), desiredRuntime: 'running' };
          const prepare = async (build, version = '0.4.0') => {
            const handoff = await protocol.createHandoff({
              sessionToken, body, appVersion: version, buildNumber: build,
            });
            return protocol.createTicket({ handoffToken: handoff.handoffToken, body });
          };
          const rawTicket = await prepare(buildNumber, appVersion);
          const ticket = JSON.parse(rawTicket).data;
          const request = {
            ...body, ticket: ticket.ticket, requestDigest: ticket.requestDigest,
            installation: {
              id: opaque('nsi_', 1), keyGeneration: 1,
              possessionPublicJwk: possession.publicKey.export({ format: 'jwk' }),
              envelopePublicJwk: envelope.publicKey.export({ format: 'jwk' }),
            },
            proof: { algorithm: 'ES256', signature: '' },
          };
          const keys = validateInstallationKeys(request.installation);
          request.proof.signature = crypto.sign('sha256', possessionTranscript({
            request, keys, ticketHash: sha256Hex(ticket.ticket),
            exchangeChallenge: ticket.exchangeChallenge, network,
          }), { key: possession.privateKey, dsaEncoding: 'ieee-p1363' }).toString('base64url');
          const exchange = () => protocol.exchange({ body: request });
          if (wallet !== 'existing-account' && !compatible) {
            await assert.rejects(exchange(), {
              code: wallet === 'no-season' ? 'native_session_no_active_season' : 'native_session_wallet_pool_exhausted',
            });
            assert.equal((await pool.query('SELECT id FROM mobile_auth_tokens WHERE user_id = $1', [userId])).rowCount, 0);
            assert.equal((await pool.query('SELECT credential_reference FROM native_session_credentials WHERE user_id = $1', [userId])).rowCount, 0);
            assert.equal((await pool.query('SELECT token FROM sessions WHERE user_id = $1', [userId])).rowCount, 1);
            // The already-built app can retry the same durable attempt on upgrade.
            assert.equal(await prepare('1252'), rawTicket);
          }
          const rawExchange = await exchange();
          const data = JSON.parse(rawExchange).data;
          const decrypted = await compactDecrypt(data.envelope.compactJwe, privateKey);
          const credential = JSON.parse(Buffer.from(decrypted.plaintext).toString('utf8'));
          assert.equal(credential.subject.userId, String(userId));
          assert.equal(credential.account?.accountId ?? null,
            wallet === 'existing-account' ? String(userId) : null);
          assert.match(credential.credential.bearerToken, /^[0-9a-f]{80}$/);
          assert.equal(await exchange(), rawExchange, 'replay retains byte-identical ciphertext');
          assert.equal((await pool.query('SELECT id FROM mobile_auth_tokens WHERE user_id = $1', [userId])).rowCount, 1);

          if (wallet !== 'existing-account') {
            for (const olderBuild of ['1250', undefined]) {
              assert.equal(await prepare(olderBuild), rawTicket);
              await assert.rejects(exchange(), { code: 'native_session_wallet_required' });
            }
            assert.equal(await prepare('1252'), rawTicket);
            assert.equal(await exchange(), rawExchange);
          }
          const validProof = request.proof.signature;
          request.proof.signature = Buffer.alloc(64).toString('base64url');
          await assert.rejects(exchange(), { code: 'invalid_native_session_possession_proof' });
          request.proof.signature = validProof;
          await pool.query('DELETE FROM sessions WHERE user_id = $1', [userId]);
          assert.equal(await exchange(), rawExchange, 'committed replay survives web session deletion');
          await pool.query(`UPDATE native_session_credentials
            SET state = 'revoked', revoked_at = NOW(), revocation_reason = 'account_recovery'
            WHERE user_id = $1`, [userId]);
          await assert.rejects(exchange(), { code: 'native_session_credential_revoked' });
        });
      }
    }
  });
