...
const ADDED_IDS = {
  ...
  'workshop-empty': 'Its nothing-to-show state, for an account with no apps. #2445 made it a CARD rather than the grey caption line it shipped as — a <ListRow as="a" href="#apps">, so it reads as an invitation to the directory the way Home\'s Discover block\'s own empty card does (#1913), title over subtitle with the row\'s disclosure chevron. THE ID AND THE TAG DID NOT MOVE: dapp.json selects `#workshop-empty.hidden`, so the id stays on the one element and visibility stays a `hidden` class toggle on it rather than conditional rendering. Ships `hidden`, and stays hidden while the list is still loading — the skeleton rows are that state, and an empty list that reads as "you have no apps" before the fetch lands is the bug this distinction prevents. It is the list\'s FIRST child, not its last: GroupedList\'s row separator is `[&:not(:last-child)]:after:*` on the row, so a note after the rows would leave the last one drawing a hairline under nothing.',
