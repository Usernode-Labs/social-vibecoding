...
const ADDED_IDS = {
  ...
  'workshop-list': 'That screen\'s rows host, rendered end to end by features/workshop/index.tsx. Empty in the prerender, like #browse-list beside it; a row is `[data-workshop-app="<slug>"]` and carries `[data-workshop-working]` and `[data-workshop-needs]`, which is what the declared checks select on.',
  'workshop-empty': 'Its nothing-to-show line, for an account with no apps. Ships `hidden` and stays hidden while the list is still loading — the skeleton rows are that state, and an empty list that reads as "you have no apps" before the fetch lands is the bug this distinction prevents.',
