...
//   - it is spawn() (streaming), so nothing buffers the whole dump;
...
test('exportFilename is timestamped, .dump, and filesystem-safe', () => {
  const name = dbExport.exportFilename('usernode', new Date('2026-07-27T12:13:14.567Z'));
  assert.equal(name, 'usernode-platform-usernode-20260727T121314Z.dump');
...
test('pg_dump is invoked as argv — no shell anywhere', async () => {
  let seen = null;
  const child = fakeChild();
  const res = fakeRes();
  const p = dbExport.runExport({
    dbName: 'usernode', res, filename: 'x.dump',
  ...
  assert.deepEqual(seen.args,
    ['exec', dbExport.DB_CONTAINER, 'pg_dump', '-U', dbExport.DB_USER, '-Fc', '--no-password', 'usernode']);
...
test('an unsafe db name is refused before anything is spawned', async () => {
  ...
  const out = await dbExport.runExport({
    dbName: 'usernode; rm -rf /', res, filename: 'x.dump',
...
test('a successful export streams bytes through and reports the count', async () => {
  const child = fakeChild();
  const res = fakeRes();
  const p = dbExport.runExport({ dbName: 'usernode', res, filename: 'dump.dump', spawnFn: () => child });
  child.emitOut('AAAA');
  child.emitOut('BBBBBB');
  child.exit(0);
  const out = await p;

  assert.equal(out.status, 'completed');
  assert.equal(out.bytesSent, 10);
  assert.equal(res.bytes(), 10, 'the bytes reached the socket, not a buffer');
...
test('download headers are set on the first byte, and no Content-Length is claimed', async () => {
  ...
  assert.equal(res.headers['content-disposition'], 'attachment; filename="dump.dump"');
...
test('onStart fires once, when the stream actually begins', async () => {
  ...
  let starts = 0;
  const p = dbExport.runExport({
    dbName: 'usernode', res, filename: 'd.dump', onStart: () => { starts++; }, spawnFn: () => child,
  });
...
test('a child that dies before the first byte still yields a JSON error', async () => {
  const child = fakeChild();
  const res = fakeRes();
  const p = dbExport.runExport({ dbName: 'usernode', res, filename: 'd.dump', spawnFn: () => child });
...
test('a mid-stream failure destroys the socket instead of ending it truthfully', async () => {
  ...
  // silently truncated dump.
  const child = fakeChild();
  const res = fakeRes();
  const p = dbExport.runExport({ dbName: 'usernode', res, filename: 'd.dump', spawnFn: () => child });
  child.emitOut('PGDMP-partial');
...
test('a spawn failure resolves rather than rejecting', async () => {
  const res = fakeRes();
  const out = await dbExport.runExport({
    dbName: 'usernode', res, filename: 'd.dump',
...
test('stderr never reaches the client body', async () => {
  ...
  child.exit(0);
  await p;
  assert.equal(res.bytes(), 1, 'only dump bytes are written to the response');
});
