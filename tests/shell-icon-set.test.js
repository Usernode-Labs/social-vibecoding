...
test('the glyphs that do NOT prerender are the ones that render behind state', () => {
  ...
  const expected = [
    ...
    // TrophyOutlineIcon is BACK on this list, for a new reason. It was the
    // Challenges block's leaderboard link glyph: fetched (absent) while the
    // link lived in the card, then prerendered once the link moved into the
    // SECTION HEADING with the rest of the block's chrome. The homescreen
    // design draws the heading's link as text alone, so the glyph has no
    // call site at all now (features/home/panels/ui.tsx) — the export stays
    // for the leaderboard surfaces to pick up. The same change took
