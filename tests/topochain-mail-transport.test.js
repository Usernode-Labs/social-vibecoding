...
test('sendWaitlistReleaseMail passes kind:"waitlist_released" and a signup/login link', async () => {
  ...
  // #1545: an existing account has nothing to carry, so it links to a QUERY,
  // not a fragment. Following `/#login` from a desktop mail client landed on
  // the home page while the same mail worked from a phone — a link rewriter
  // rebuilding the URL drops a fragment and keeps a query. AuthScreens.enter()
  // turns the query spelling into the same hash route.
  assert.match(seen[1].url, /\/\?login=1$/, 'existing account → the link lands on sign-in');
  assert.doesNotMatch(seen[1].url, /#/, 'nothing a link rewriter is free to drop');
});
