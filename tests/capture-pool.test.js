...
test('pool bounds come from env with sane defaults and a hard ceiling', () => {
  ...
  // 520000 since #1489 (470000 since #1417), moved with MAX_DECLARED_TESTS
  // 480 → 530 and the platform-side default in services/visuals.js. The
  // three are asserted equal to each other elsewhere
  // (tests/checks-budget.test.js); this one pins that the container's own
  // fallback is the raised value, so a run without the env var does not
  // quietly apply the old shorter budget.
