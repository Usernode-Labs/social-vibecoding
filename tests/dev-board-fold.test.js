...
test('the declared checks that read a board card’s anatomy run with the cards open; two pin the fold', () => {
  ...
  // 636 → 637: #1933's one board check above, merged onto that 636. Counted
  // off the merged manifest again rather than re-derived: 637.
  assert.equal(DAPP.tests.length, 637);
});
