...
- The set moved because the old one disagreed with itself: Heroicons is four optical families
  keyed by size, and v2 redrew every path AND moved outline weight 2 → 1.5, so a set hand-picked
  across that boundary shipped three plusses, two checks, and a paperclip drawn twice at
  different decimal precision (#1120 recorded them and, as a mechanical refactor, could not
  change pixels). One family with no version boundary to accrete across is the fix.
- OpenMoji is the other half and does NOT compete with it: lucide is INTERFACE, OpenMoji is
  ILLUSTRATION. The curated 290-file slice (`public/vendor/openmoji/`, pinned to
  `openmoji@17.0.0`) is selected by a reviewable emoji-literal table in
  `scripts/vendor-assets.js`, and the vendor run FAILS LOUDLY if a literal stops resolving
  against the pinned package. `frontend/src/lib/openmoji.js` is the lookup, and a miss is a
  soft degrade to the platform's text-emoji rendering — never a broken tile.
...
- `tests/tailwind-build.test.js` performs a fresh compile into a temporary
  ...
  artifact.
- The shell loads **no cross-origin assets**. Every vendored family lives
  under `public/vendor/`, put there by `npm run vendor:assets` (provenance in
  `public/vendor/README.md`). The enumeration that used to sit here is
  deliberately gone: it named three families, went stale the moment a fourth
  was added, and the README is the list that is generated rather than typed.
  Don't add a CDN `<script>`/`<link>` to `frontend/src/head.html` — vendor or
  compile it instead; three tests enforce this.
