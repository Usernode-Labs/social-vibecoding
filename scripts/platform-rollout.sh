...
#
# Invoked by .github/workflows/deploy.yml after the .env write + infra
# `compose up`. Safe to re-run: it always targets whichever color is NOT
# currently live, so a retried/failed rollout converges.
#
# Caddy's active-color file is the source of truth for "which color is live":
# caddy/active/platform-upstream.caddy. It's committed pointing at blue (the
# cold-start default) and rewritten here on every flip.
...
# healthcheck uses it too) until it answers 200 or we time out.
wait_healthy() {
  local name="$1"
