...
// orchestrator's 620s docker timeout so the container always gets to emit
// its sentinel rather than being killed mid-suite.
function testsDeadlineMs(env) {
  // 470000 → 490000, moved with MAX_DECLARED_TESTS 480 → 500 and the
  ...
  return (Number.isFinite(raw) && raw > 0) ? raw : 490000;
}
...
async function runTests(browser, tests, opts) {
  ...
  const budgetMs = Number(o.deadlineMs) > 0 ? Number(o.deadlineMs) : 490000;
