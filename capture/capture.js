...
function testsDeadlineMs(env) {
  // 420000 → 470000 (#1417) → 520000, moved with MAX_DECLARED_TESTS
  // 430 → 480 → 530 and the platform-side TESTS_DEADLINE_MS. The two
  // defaults are asserted equal by tests/checks-budget.test.js precisely so
  // a container running without the env var cannot silently apply a shorter
  // budget than the platform planned — which would cut a full manifest's
  // tail while the platform reported the suite as merely unfinished.
