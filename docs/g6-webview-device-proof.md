...
| React source/build | **Blocked:** the browser/pipeline implementation currently ends at `0e5617c`, incorporating host contracts (`6785617`), worker/session isolation (`2eca16f`), exact-artifact deployment (`db0ea79`), deliberate legacy-shortcut deferral (`aad39af`), and complete authority gates (`ab2034d`). No final candidate SHA has yet produced and deployed a retained verified artifact. Replace this cell with that final Git SHA, verified-artifact manifest/checksum SHA-256, and deployed `/react/react-sw.js` SHA-256 before testing |
